...
> `bakeoff-prompts.json` (beside this file, in `docs/research/`), and verify with the command at the bottom before you spend money.
...
```bash
python3 -c "import json;[print(a['arm'].ljust(3),[hex(ord(c)) for c in a['name_line'] if ord(c) in (0x2bb,0x2018,0x2019,0x27,0x60,0xb4)] or 'no-modifier') for a in json.load(open('docs/research/bakeoff-prompts.json'))['arms']]"
```
