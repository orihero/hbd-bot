...
> Generated from a 15-agent research pass over the HBD codebase: 8 parallel researchers, synthesis into one catalog, then a 5-way adversarial verification pass that re-opened every referenced model file to confirm each `table.column` actually exists.
...
| **Deployment capability ledger** | Which numbers on this screen are absences rather than zeros? | `breakdown` | list — *Instrumentation* | `CONFIRMED end to end, all eight flags. src/hbd/db/admin/metrics.py:214-225 returns…` | 🟢 served today | **T1** |
...
| **Orders per day (volume and outcome)** | How much work arrived each day, how did it end, and is today normal? | `timeseries` | F11 Tick Gauge — *Delivered, last 24h* | `Confirmed exactly as stated. orders_per_day (src/hbd/db/admin/metrics.py) selects…` | 🟢 served today | **T1** |
...
| **Abandoned-draft backlog** | How many wizard sessions were opened and never confirmed? | `single-number` | F1 Rung Bars — *Where the whole book sits* | `ALREADY SHIPPED — do not build it. hbd.db.purge.rows_past_expiry_statements…` | 🟢 served today | T2 |
...
| **Failures per day (error timeseries)** | When did this class of failure start, and is it new? | `timeseries` | G10 Diverging Bar — *Failure codes vs baseline* | `Correct as stated. failure_breakdown at src/hbd/db/admin/metrics.py:155-182 already does…` | 🟡 one query away | T2 |
...
| **Order failure reasons** | When an order dies, what killed it? | `ranked-list` | G10 Diverging Bar — *Failure codes vs baseline* | `CONFIRMED. orders.failed_reason exists — src/hbd/db/models/order.py, `failed_reason:…` | 🟡 one query away | T2 |
...
| Metric | The question it answers | Shape | Visualisation | Source | Avail. | Tier |
|---|---|---|---|---|---|---|
| **Bookings by day (plan purchases)** | How much money came in? | `timeseries` | F1 Rung Bars — *Bookings, last 8 weeks (stub rail)* | `CONFIRMED column for column on src/hbd/db/models/plan_purchase.py: `telegram_user_id`…` | 🟠 new endpoint | **T1** |
| **Cost per delivered song** | What does one delivered kit actually cost us in vendor fees? | `distribution` | F11 Tick Gauge — *Cost per delivered song* | `Every column checked and CONFIRMED on src/hbd/db/models/vendor_usage.py: `cost_usd`…` | 🟠 new endpoint | **T1** |
...
| **Priced coverage of the spend figure** | How complete is the dollar number I am reading? | `single-number` | stat — *Vendor spend, 30d* | `CONFIRMED. VendorUsageTotals.costed_calls: int at src/hbd/db/admin/views.py:982 (the…` | 🟢 served today | **T1** |
...
| **Effective revenue per song by product** | What does a song actually sell for once plan buyers use their allowance? | `distribution` | stat — *Price shipped vs cost* | `Every column verified on src/hbd/db/models/plan_purchase.py: amount_minor (Integer,…` | 🟠 new endpoint | T2 |
...
| **Plan utilisation and breakage** | Do customers use what they paid for? | `distribution` | F14 Rung Histogram — *Plan utilisation and breakage* | `CONFIRMED, every column. src/hbd/db/models/plan_purchase.py: songs_included (Integer,…` | 🟠 new endpoint | T2 |
...
| **Token, character and audio-minute consumption** | What physical volume are we buying from each leg? | `timeseries` | table — *Vendor × operation × model* | `All five columns verified on vendor_usage (src/hbd/db/models/vendor_usage.py):…` | 🟡 one query away | T3 |
...
| **Outstanding credit liability** | How many unspent credits are sitting on customer balances? | `single-number` | stat — *Plan liability* | `CONFIRMED. src/hbd/db/models/credit_account.py: balance (Integer, NOT NULL, default 0,…` | 🟠 new endpoint | T2 |
...
| **Lyric-budget pressure** | How many accounts are hitting the daily lyric-write ceiling? | `single-number` | F7 Stacked Rungs — *How charges close* | `CONFIRMED with one arithmetic correction. src/hbd/db/models/lyric_budget.py:…` | 🟠 new endpoint | T3 |
...
| **Vendor x operation x model rollup** | Which vendor leg is burning the money, and at what unit? | `matrix` | table — *Vendor × operation × model* | `CONFIRMED, every field. Route: src/hbd/admin/routers/vendors.py:94 VENDOR_USAGE_PATH =…` | 🟢 served today | T2 |
...
| **Vendor HTTP status mix** | Are failures rate limits, auth problems, or transport drops? | `breakdown` | L4 Arc Matrix — *Errors by vendor and code* | `CONFIRMED. src/hbd/db/models/vendor_usage.py: `http_status: Mapped[int \| None] =…` | 🟡 one query away | T3 |
...
| **Name strategy bake-off** | Which candidate orthography should HBD_NAME_CANDIDATE_ORDER put first? | `ranked-list` | F5 Tick Rows — *Strategy bake-off* | `CONFIRMED, with the index claim overstated. `NAME_STRATEGIES_PATH =…` | 🟢 served today | T2 |
| **Audio quality outliers** | Are we shipping songs that are too short, too quiet or malformed? | `distribution` | — | `Columns confirmed on src/hbd/db/models/asset.py: `duration_s` (Float, **nullable=False,…` | 🟠 new endpoint | T3 |
...
| **Occasion / genre / language mix** | What are customers actually ordering, and in which language? | `breakdown` | F10 Dot Heat — *Genre × language pass rate* | `Columns CONFIRMED on src/hbd/db/models/brief.py — `occasion`, `genre`, `vocal_gender`,…` | 🟠 new endpoint | T2 |
...
| **Lyrics-preview volume vs orders** | Do people who try the free lyric writer come back and buy? | `relationship` | L17 Calendar Heat — *The occasion calendar* | `UsageTask.LYRICS_PREVIEW confirmed at src/hbd/contracts.py:333, and its docstring…` | 🟡 one query away | T3 |
...
| Metric | The question it answers | Shape | Visualisation | Source | Avail. | Tier |
|---|---|---|---|---|---|---|
| **Retention backlog by clock** | How many rows are past their retention clock right now? | `breakdown` | stat — *Rows past their clock* | `CONFIRMED, all twelve keys, verbatim. src/hbd/admin/schemas/retention.py:59-78…` | 🟢 served today | **T1** |
...
| **Login and step-up failures** | Is the panel under credential pressure? | `timeseries` | F1 Rung Bars — *Reveals and denials by operator* | `Actions CONFIRMED verbatim in src/hbd/db/enums.py: `LOGIN_FAILURE = "login.failure"`…` | 🟠 new endpoint | T2 |
...
| **Retention sweep health** | Did the purge run, did it finish, and is it falling behind? | `timeseries` | stat — *Last sweep* | `Every field verified against src/hbd/admin/schemas/retention.py: RetentionResponse…` | 🟢 served today | T2 |
...
| **Admin roster health** | Is the console's own access hygiene sound? | `breakdown` | table — *Sweep history, last 24 runs* | `CONFIRMED field for field. `GET /api/admins` is src/hbd/admin/routers/admins.py under…` | 🟢 served today | T3 |
| **Admin session posture** | Who is logged in, who is elevated, and are sessions being cleaned up? | `single-number` | F1 Rung Bars — *Reveals and denials by operator* | `Every column verified on src/hbd/db/models/admin_session.py: revoked_at (nullable),…` | 🟠 new endpoint | T3 |
...
| **Asset storage footprint** | How many bytes are we holding, and of what? | `breakdown` | table — *Sweep history, last 24 runs* | `CONFIRMED. src/hbd/db/models/asset.py: kind (AssetKind enum, :50), size_bytes…` | 🟠 new endpoint | T3 |
| **Audit chain integrity** | Can the tamper-evident audit log still be trusted? | `single-number` | stat — *Audit chain* | `CONFIRMED, all seven fields. src/hbd/admin/schemas/audit.py:118-133 ChainVerifyResponse:…` | 🟢 served today | T3 |
...
4. plan_purchases HAS NO ADMIN SURFACE. The table holds telegram_user_id, plan, songs_included, songs_used, amount_minor, currency, provider, reference, idempotency_key and plan_ends_at with two indexes, and grep finds zero references to PlanPurchaseRow under src/hbd/admin/ or src/hbd/db/admin/. Needs db/admin/plans.py, schemas/plans.py and routes before any revenue, plan-liability, utilisation or deferred-revenue tile can exist.
