...
the operator built on the records screen, and that filter is a *document* (``hbd.db.admin.
...
:class:`~hbd.db.enums.AuditReasonCode` and the bounded ticket reference carry the
...
from hbd.contracts import BroadcastKind, BroadcastState
from hbd.db.base import SHA256_LENGTH, Base, TimestampMixin, UtcDateTime, enum_type
from hbd.db.enums import AuditReasonCode
from hbd.db.models.admin_audit import ACTOR_USERNAME_LENGTH, REASON_REF_LENGTH
...
#: A base64url keyset cursor from ``hbd.db.admin.page``. 256 is comfortably above the encoded
...
#: ``HbdError.error_code`` is a short symbolic name, never a message. Matches
...
class BroadcastRow(TimestampMixin, Base):
    ...
    **No foreign key to ``admin_users``**, following ``admin_audit_log``'s reasoning one step
    ...
    #: :class:`hbd.contracts.BroadcastKind` for why collapsing the two would make widening
    ...
    #: Terminal failure of the RUN, not of a recipient — a symbolic ``hbd.errors`` name, never
