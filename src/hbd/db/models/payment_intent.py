...
``hbd.db.purge.rows_past_expiry_statements`` reads it. That suffix is this codebase's word
...
and ``hbd.checkout.PlanState.ends_at`` are named for the identical reason.
...
cutoff by ``hbd.db.purge`` — bounded growth over a table that gets a row every time anybody
...
from hbd.db.enums import IntentProduct, PaymentIntentState
...
#: migration, and a rail that one day requires a prefix (``hbd-``) or a checksum suffix must
...
#: whose provider validated in ``hbd.checkout`` cannot fail its INSERT here or its copy into
#: the receipt.
