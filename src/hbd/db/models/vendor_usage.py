...
* **Its quantity columns default to zero.** ``cost_usd`` defaults to ``0.0`` and
  ...
  and reads as "free". That is precisely why ``src/hbd/admin/routers/dashboard.py`` refuses
...
precedent: ``hbd.db.purge.VENDOR_USAGE_RETENTION_DAYS`` deletes rows past 400 days, a
...
#: Mirrors ``purge_runs.error_code``: ``HbdError.error_code`` is a short symbolic name from
...
class VendorUsageRow(Base):
    ...
    #: True under ``HBD_USE_FAKE_PROVIDERS``. A demo run is RECORDED and visibly excluded
    ...
    #: From the closed ``hbd.errors`` taxonomy, never a body excerpt: a vendor error body
    ...
    #: (``HBD_ELEVENLABS_USD_PER_CHARACTER=0``), both LLM legs and transcription write ``NULL``
