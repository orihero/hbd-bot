...
Split out of :mod:`hbd.db.credits` on the seam the two halves already had — that module
...
existing transaction and :class:`hbd.db.credits.SqlCreditLedger` can wrap the rest.
...
async def settle_stale_debits(
    ...
    *,
    ...
    Written in the ``hbd.db.admin`` idiom — session first, ``now`` injected, exceptions
