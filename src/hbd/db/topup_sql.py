...
The exact sibling of :mod:`hbd.db.plan_sql`, and written to the same rule: nothing here
...
lives in :mod:`hbd.db.purchases`, so that "what does a paid single song write?" reads as a
...
facts, and both are written under ONE key.** ``hbd.db.purchases._fulfil_single`` writes a
``topup_purchases`` row here and a ``credit_ledger`` GRANT through
:func:`hbd.db.credits.grant`, in one transaction, stamped from one clock, keyed on one
...
which a receipt has no business asserting. :mod:`hbd.db.models.topup_purchase` argues at
length why the money did not simply become four more columns on ``credit_ledger``.

**:func:`hbd.db.credit_sql.insert_or_ignore` is the whole concurrency story.** Two writers
...
These names are public to the ``hbd.db`` package and to nothing else, exactly like
...
async def topup_by_key(session: AsyncSession, idempotency_key: str) -> TopupPurchaseRow | None:
    ...
    :func:`hbd.db.plan_sql.plan_by_key` exists because the plan's caller must hand the
...
async def insert_topup(
    ...
    *,
    ...
    ``amount_minor`` is written verbatim from :class:`hbd.checkout.Purchase` and is never
...
async def anonymise_topups(session: AsyncSession, *, telegram_user_id: int) -> int:
    ...
    :mod:`hbd.db.credit_erasure`'s: a receipt is an audit fact that answers "was this
    ...
    :func:`hbd.db.plan_sql.anonymise_plans` and the ledger's own treatment. It is the replay
