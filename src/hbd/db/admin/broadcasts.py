...
stored document travels on :class:`~hbd.db.admin.views.BroadcastDetail` verbatim, because a
recompiled segment answers "who would match NOW" — a different question, and on a campaign
that has already gone out a misleading one. :mod:`hbd.db.admin.segment` is therefore
...
**What crosses, and under whose authority.** :class:`~hbd.db.admin.views.BroadcastRecipientItem`
carries a Telegram id, which makes this the third read in ``hbd.db.admin`` that names a
...
from hbd.db.admin.sql import TimeWindow, apply_in, apply_search, apply_window
...
class BroadcastFilters:
    ...
    #: an empty page — :func:`~hbd.db.admin.sql.search_clause`'s asymmetry.
...
async def get_broadcast(session: AsyncSession, broadcast_id: UUID) -> BroadcastDetail | None:
    ...
    travel beside it on :attr:`~hbd.db.admin.views.BroadcastDetail.broadcast` — both, on
