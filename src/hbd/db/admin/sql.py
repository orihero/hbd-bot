...
class TimeWindow:
    ...
    **``start`` is optional and ``end`` is not**, which looks lopsided until you ask what the
    ...
    in :func:`hbd.admin.window.resolve_window` and this type never sees a fabricated bound.
...
class SeriesGrain(StrEnum):
    ...
    **There is deliberately no ``WEEK`` or ``MONTH`` member**, and the omission is the whole
    ...
    never by rows, which is the same licence :func:`~hbd.db.admin.metrics.failure_breakdown`
