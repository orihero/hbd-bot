...
from hbd.db.admin.views import VendorBalanceState
...
async def has_polled_vendor_balances(session: AsyncSession) -> bool:
    ...
    A ROW probe rather than a :func:`~hbd.db.admin.sql.has_table` one, for the reason the
    ...
    worker is an older build, or one where ``HBD_VENDOR_BALANCE_POLL_ENABLED`` is off.
    """
