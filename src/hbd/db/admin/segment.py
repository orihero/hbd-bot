...
It imports no FastAPI, no pydantic wire model and nothing from ``hbd.admin`` — the dependency
runs the other way, and the wire models in ``hbd.admin.schemas.segment`` lower onto the plain
...
from hbd.checkout import PaymentIntentState, PlanStatus
from hbd.contracts import BotMembershipEvent, Err, Language, OrderState, Result, err, is_err, ok
from hbd.db.admin.sql import CHAT_MESSAGES_TABLE, has_table
from hbd.db.base import UtcDateTime
from hbd.db.enums import ChatDirection
from hbd.db.models.bot_membership_event import BotMembershipEventRow
from hbd.db.models.chat_message import WIZARD_STEP_LENGTH, ChatMessageRow
...
class SegmentError(ValidationError):
    ...
    Its own class rather than a bare :class:`~hbd.errors.ValidationError` so the boundary can
