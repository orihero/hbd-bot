...
from hbd.db.admin.balances import has_polled_vendor_balances
from hbd.db.admin.overview import has_recorded_activity_history, has_recorded_churn
from hbd.db.admin.plan_purchases import has_recorded_plan_revenue
...
from hbd.db.admin.topup_purchases import has_recorded_topup_revenue
from hbd.db.admin.vendor_usage import has_priced_vendor_usage, has_recorded_vendor_usage
...
async def delivered_counts(session: AsyncSession, *, window: TimeWindow | None = None) -> Trend:
    ...
    predicate spanning both ranges (:func:`~hbd.db.admin.sql.count_pair` over
    :func:`~hbd.db.admin.sql.spanning_window`), so the database makes one index range scan
...
async def order_funnel(session: AsyncSession, *, window: TimeWindow | None = None) -> OrderFunnel:
    ...
    **There is no "abandoned" count**, and that absence is argued rather than overlooked:
    ...
    Not :func:`~hbd.db.admin.orders.count_orders_by_state`, which is correct but takes
...
async def read_capabilities(session: AsyncSession) -> ReadCapabilities:
    ...
    **Thirteen flags now, and five of them arrived with the dashboard instrumentation.**
    ...
    :func:`~hbd.db.admin.sql.has_table` one, for the reason the vendor pair already
    ...
    **The two vendor flags are ROW probes and not** :func:`~hbd.db.admin.sql.has_table`
    **ones**, which is the only place on this surface where the difference is load-bearing.
    ...
    complete" — which is why :func:`~hbd.db.admin.vendor_usage.vendor_usage_rollup` sends
