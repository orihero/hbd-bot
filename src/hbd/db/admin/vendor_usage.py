...
The rule :mod:`hbd.db.admin.metrics` states holds here without exception: **every number is
...
:func:`~hbd.db.admin.sql.count_where` renders ``count(CASE WHEN … END)``, which is ``0`` for
...
:func:`~hbd.db.admin.attempts.is_cost_instrumented`. "No rows in the range you chose" and
...
Before it existed, a ``HBD_USE_FAKE_PROVIDERS`` demo run inflated ``costUsd``, ``calls``,
...
filter. (``hbd.db.vendor_balances`` is the one non-reporting reader and it spells
...
from hbd.contracts import CostSource, Vendor, VendorOperation
...
#: imported from ``hbd.db.admin.metrics``, which imports this module.
...
async def vendor_usage_per_day(
    ...
    *,
    ...
    deployment existed. The day is :class:`~hbd.db.admin.sql.UtcDay` and never
...
async def vendor_error_breakdown(
    ...
    *,
    ...
    :func:`~hbd.db.admin.metrics.failure_breakdown`: retryability belongs to the
    ``HbdError`` subclass that was raised and not to the string that got persisted, so
...
async def cost_per_delivered_song(
    ...
    **The cohort is ``delivered_at`` and not ``created_at``, and that overrules the research
    ...
    :func:`~hbd.db.admin.metrics.delivered_per_bucket`,
    :func:`~hbd.db.admin.metrics.delivered_latency` — is windowed on ``delivered_at``.
...
async def cost_per_delivered_song_by_vendor(
    ...
    * ``None`` is not an addend. A vendor whose every call in the window was unpriced comes
    ...
    whose group count is bounded by :class:`~hbd.contracts.Vendor`, not by the table.
    """
...
async def units_per_delivered_song_by_vendor(
    ...
    **This shape carries no coverage count, and the missing number is recoverable.**
    :class:`~hbd.db.admin.views.VendorCostPerSong` publishes ``attributed_orders`` beside
    its cost precisely so an average over the instrumented subset cannot be read as an
    average over the cohort; :class:`~hbd.db.admin.views.VendorUnitsPerSong` has one count
...
async def vendor_operation_latency(
    ...
    *,
    ...
    ``ORDER BY … LIMIT 1 OFFSET k``, for the reason :mod:`hbd.db.admin.metrics` gives at
...
async def has_recorded_vendor_usage(session: AsyncSession) -> bool:
    ...
    A row probe rather than a :func:`~hbd.db.admin.sql.has_table` one, and the window is
...
async def has_priced_vendor_usage(session: AsyncSession) -> bool:
    ...
    unpriced until somebody configures a rate (``HBD_ELEVENLABS_USD_PER_CHARACTER`` and the
    four ``HBD_LLM_*_USD_PER_MILLION_*`` all ship ``0.0``, and Scribe has no rate to
...
def _narrow[R: tuple[Any, ...]](
    ...
    *,
    ...
    default: :func:`~hbd.db.admin.sql.apply_window` emits no lower predicate when ``start``
    is ``None``, and :func:`~hbd.db.admin.sql.apply_in` treats an empty tuple as "no filter"
    rather than as ``IN ()``.
