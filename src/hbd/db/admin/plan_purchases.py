...
half; :mod:`hbd.db.admin.topup_purchases` is its sibling for the other product, one module
...
:data:`~hbd.db.admin.page.TOTAL_COUNT_CAP` and ``bounded_total`` are deliberately NOT used:
...
async def subscription_churn(
    ...
    **``now`` is a required parameter and not an alias for the window's end**, because the
    ...
    :func:`~hbd.db.admin.users.get_user_detail` — so a test can move it and a request's
...
async def has_recorded_plan_revenue(session: AsyncSession) -> bool:
    ...
    :func:`~hbd.db.admin.vendor_usage.has_recorded_vendor_usage` is: this table's migration
