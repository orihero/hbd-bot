...
from :func:`hbd.db.credits.unenforced_key_prefix` and asks once per page — one extra round
...
#: Refunds and settlements, counted separately. :func:`~hbd.db.admin.sql.count_where` rather
...
async def count_orders_by_state(
    ...
    **This is not a page statistic and it is not bounded.** The Orders hub's distribution bar
    ...
    :func:`~hbd.db.admin.page.bounded_total`'s ``LIMIT`` trick cannot be borrowed here,
    ...
    Zero-filled over every :class:`~hbd.contracts.OrderState` in declaration order, so the
    ...
    operator's cursor. :class:`~hbd.db.admin.views.OrderStateTotal` argues why this is the
...
def _search_predicate(text: str | None) -> ColumnElement[bool] | None:
    ...
    Not :func:`~hbd.db.admin.sql.apply_search` alone, because ``orders.id`` cannot be in
    ...
    blank-means-no-filter asymmetry — is :func:`~hbd.db.admin.sql.search_clause`'s.
    """
...
async def _unenforced_orders(session: AsyncSession, order_ids: Sequence[UUID]) -> frozenset[UUID]:
    ...
    The keys are escaped through :func:`~hbd.db.admin.sql.escape_like` even though a UUID
