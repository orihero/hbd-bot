...
decisions" layer that could not have held a lock. :mod:`hbd.payme.rules` holds every decision
...
is the lock — see :mod:`hbd.db.payme_sql`, which argues the primitive and names the two
...
object is handed to the BOT typed as :class:`hbd.checkout.PaymentIntentOpener`, which declares
``open_intent`` and nothing else, and to the GATEWAY typed as
:class:`hbd.payme.ports.PaymeLedger`, which declares the settlement half. The bot cannot
...
**Every public method is ``run_guarded`` and every refusal is an ``HbdError``.** The
:mod:`hbd.payme.errors` hierarchy inherits ``HbdError`` specifically so the guard's existing
...
``hbd.db.enums`` mirrors ``hbd.checkout`` and ``hbd.payme.protocol`` value-for-value rather
than importing them (a mapped column naming a type from outside persistence is how
``hbd.contracts`` acquired the import cycle ``hbd.entitlements`` documents). This is the one
...
from hbd.checkout import PaymentIntent, Product, Purchase
from hbd.checkout import PaymentIntentState as CheckoutIntentState
...
from hbd.payme.ports import (
    PAYME_PROVIDER_NAME,
    PaymeStatementRow,
    PaymeTransactionView,
    SettlementCounts,
)
from hbd.payme.protocol import DEFAULT_ACCOUNT_FIELD, CancelReason, PaymeErrorCode
from hbd.payme.protocol import PaymeState as ProtocolPaymeState
from hbd.payme.rules import DEFAULT_TRANSACTION_TIMEOUT_MS, account_fault_for, is_expired
...
#: other alone.
#:
#: It is a DEFAULT and not a rule — the composition root passes ``HBD_PAYME_INTENT_TTL_S`` —
...
_OPERATOR_SETTLE_PREFIX: Final[str] = "operator:"


def _intent_view(row: PaymentIntentRow) -> PaymentIntent:
    """The mapped intent as everything outside ``hbd.db`` sees it (Rule 15).

    The two enum conversions go through ``.value`` rather than through a lookup table, matching
    ``hbd.db.purchases._plan_state``: the persistence enums mirror the contract enums
...
def _transaction_view(row: PaymeTransactionRow, *, intent_public_ref: str) -> PaymeTransactionView:
    ...
    only at the wire, by :func:`hbd.payme.rules.wire_time`. Coercing earlier would make a
    ...
    * ``duplicate_code`` — the one place the rail's own materials contradict each other. The
    ...
    async def _create(
        ...
        *,
        ...
        3. **It does not exist and the intent is payable.** The row is inserted and then
           :func:`hbd.db.payme_sql.hold_intent` is what actually decides: ``rowcount == 0``
    ...
    async def _perform(self, *, payme_transaction_id: str, now: datetime) -> PaymeTransactionView:
        ...
        8. The sale, through the SHARED write primitive
           (:func:`hbd.db.fulfilment.write_single_sale`) under the intent's OWN bot-minted key.
    ...
    async def _expire_stale_transactions(self, *, now: datetime, limit: int) -> int:
        ...
        once through :func:`hbd.payme.rules.is_expired` to decide. The SQL comparison is
    ...
    async def _force_settle(self, *, public_ref: str, now: datetime, note: str) -> PaymentIntent:
        ...
        **Already paid is a no-op that says so**, returning the existing intent with its
        ...
        :func:`hbd.db.payme_sql.claim_intent_for_operator`'s state terms.
        ...
        async with self._sessions.begin() as session:
            ...
            claimed = await claim_intent_for_operator(
                ...
                intent_id=row.id,
                now=now,
                note=f"{_OPERATOR_SETTLE_PREFIX}{note}"[:SETTLE_NOTE_LENGTH],
            )
            if not claimed:
                raise PaymentError(
    ...
    async def _write_sale(
        ...
        *,
        ...
        :func:`hbd.db.fulfilment.write_plan_sale` gained a product guard during the extraction
    ...
    def _payable_intent_or_refuse(
        ...
        * a terminal intent state → ``-31051`` paid, ``-31052`` cancelled, ``-31053`` expired,
          through :func:`hbd.payme.rules.account_fault_for`, which returns ``None`` for the two
