...
:attr:`~hbd.contracts.BroadcastRecipientState.UNKNOWN`, where it counts as neither sent nor
...
state = 'pending'`` and a ``rowcount`` of 1 *is* the claim — the idiom :mod:`hbd.db.churn`
...
:func:`~hbd.db.credit_sql.insert_or_ignore` against
...
:attr:`~hbd.contracts.BroadcastRecipientState.SKIPPED_BLOCKED` at expansion, *inserted rather
...
Shape follows :mod:`hbd.db.churn` and :mod:`hbd.db.lyric_budget`: the module-level functions
...
from hbd.db.admin.page import Cursor, decode_cursor, encode_cursor, keyset_order, keyset_predicate
...
class NewBroadcast:
    ...
    :func:`~hbd.db.admin.users.count_segment_exactly` returned at creation, the second is the
...
class ClaimedRecipient:
    ...
    the caller settles it :attr:`~hbd.contracts.BroadcastRecipientState.UNDELIVERABLE`
...
class BroadcastPlan:
    ...
    One read rather than an ORM load of :class:`~hbd.db.models.broadcast.BroadcastRow`,
...
async def create_broadcast(session: AsyncSession, *, draft: NewBroadcast, at: datetime) -> UUID:
    ...
    Core inserts rather than ``session.add`` for :mod:`hbd.db.churn`'s reason: the caller
...
async def expand_chunk(
    ...
    *,
    ...
    ``predicate`` is :func:`~hbd.db.admin.segment.compile_segment`'s output for the stored
    ...
    **The walk is the account order, never the campaign's display sort.** ``(users.created_at
    ...
    The insert goes through :func:`~hbd.db.credit_sql.insert_or_ignore` against
...
async def settle_recipient(
    ...
    *,
    ...
    ``error_code`` is a symbolic token: an :mod:`hbd.errors` member or a Telegram error
...
async def release_recipient(session: AsyncSession, *, recipient_id: UUID, at: datetime) -> bool:
    ...
    **The one route from ``SENDING`` back to ``PENDING``, and it is narrow on purpose.** A
    ...
    settle the row :attr:`~hbd.contracts.BroadcastRecipientState.UNKNOWN` instead, or leave
...
class SqlBroadcasts:
    ...
    The same division of labour :class:`~hbd.db.churn.SqlBotBlocks` keeps: the module
    ...
    ``Err`` through :func:`~hbd.db.guard.run_guarded`, so no job body carries a ``try``.
