...
The sibling of :mod:`hbd.db.credit_sql`, and written to the same rule: nothing here decides
...
:mod:`hbd.db.purchases` (buying a plan) and :mod:`hbd.db.credits` (spending one), so that
...
``hbd.db.credits._mint_due_allowance`` already uses for the rolling allowance. The
...
async def current_plan(
    ...
    still yes until the end date arrives. ``hbd.db.purchases.start_plan`` refuses to sell a
...
async def anonymise_plans(session: AsyncSession, *, telegram_user_id: int) -> int:
    ...
    argued in :mod:`hbd.db.credit_erasure`: a receipt is an audit fact that answers "was this
