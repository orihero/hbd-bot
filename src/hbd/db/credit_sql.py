...
Split out of :mod:`hbd.db.credits` so that the *policy* (when to mint, when to refuse, what
...
These names are public to the ``hbd.db`` package and to nothing else: ``hbd/db/__init__.py``
...
def rowcount_of(result: object) -> int:
    ...
    all this module relies on — ``hbd.db.purge`` counts ids instead precisely because it
...
async def account_exists(session: AsyncSession, telegram_user_id: int) -> bool:
    ...
    is :func:`hbd.db.credits.refund`: after ``/forget`` the row is gone while the debit it
...
def period_grant_key(telegram_user_id: int, index: int) -> str:
    ...
    (:func:`hbd.db.credits._mint_due_allowance`) and the read that predicts it
...
async def read_balance(
    ...
    *,
    ...
    key on the anonymised ledger row (``hbd.db.credit_erasure``, which argues why: the key
...
#: :func:`hbd.db.credits.settle_stale_debits` CONSUMES it instead.
...
class StaleDebit:
    ...
    ``hbd.db.credits``, which re-derives the amount and the generation from the ledger
...
async def stale_debits(
    ...
    **"Quiet" means the ORDER ROW has not moved either, and that half is load-bearing.**
    ...
    the cron caller and the ledger resolve different policies (``hbd.runtime.retention_job``
