...
from hbd.db.churn import anonymise_bot_membership_events
...
from hbd.db.plan_sql import anonymise_plans
from hbd.db.topup_sql import anonymise_topups
...
async def forget_account(session: AsyncSession, *, telegram_user_id: int) -> CreditErasure:
    ...
    **What this does not reach.** A song already in the studio settles after the erasure,
    ...
    # being asserted by test — on a deployment where ``HBD_CHECKOUT_PROVIDER=stub`` and no
