"""The persisting :class:`~hbd.usage.UsageSink`: one ``vendor_usage`` row per vendor call.
...
(:class:`~hbd.usage.VendorUsage`), and this module reads who it was for from
...
because :func:`~hbd.usage.log_usage` runs BEFORE the insert is attempted, so a database
...
from hbd.logging import current_correlation_id, get_logger
from hbd.usage import UsageContext, VendorUsage, current_usage_context, log_usage

__all__ = ["DbUsageSink"]

_log = get_logger(__name__)

#: What ``hbd.logging`` returns when no correlation id has been bound. Stored as ``NULL``
...
class DbUsageSink:
    ...
    :class:`~hbd.runtime.providers.ProviderSet`. Each ``record`` opens and commits its own
...
def _to_row(usage: VendorUsage, context: UsageContext) -> VendorUsageRow:
    ...
    the model without adding a field to :class:`~hbd.usage.VendorUsage` (or the reverse) is
