...
worker a customer write, the coupling :class:`hbd.db.user_profiles.SqlUserProfiles` and
``KitRepository`` are kept apart to avoid.

**Why an UPSERT and not the SELECT-then-INSERT it replaces.** ``credits.touch`` writes this
same row from ``hbd.bot.gate.TouchDrain``, which runs on its own task OFF the per-chat FSM
...
``IntegrityError``, which ``hbd.db.guard.run_guarded`` classifies as TERMINAL and never
...
The shape is the module-level half of the split :mod:`hbd.db.credits` and
:mod:`hbd.db.lyric_budget` both make: session first and positional, ``now`` as a parameter,
...
#: a reader can find; :mod:`hbd.db.credits` declares the same value for the same reason and
...
async def ensure_user(
    ...
    **``is_language_authoritative`` is the whole point of the signature.** An order is
    ...
    ``last_seen_at`` and which ``hbd.bot.gate`` cites out loud at ``gate.py:124-128``.
    ...
    function would be the same defect ``hbd.db.credits.touch`` was fixed for, wearing a
    ...
    **``ui_language`` is ``Language | None`` because some callers honestly know nothing
    ...
    authority over a value it does not have — so it raises :class:`hbd.errors.ValidationError`
