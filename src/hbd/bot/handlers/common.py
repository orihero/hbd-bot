...
from hbd.bot.draft import ONBOARDED_KEY, UI_LANGUAGE_KEY, WizardDraft, load_draft
from hbd.bot.i18n import FALLBACK_LANGUAGE, translate
from hbd.bot.keyboards import MENU_LABELS, start_over_keyboard
...
if TYPE_CHECKING:  # pragma: no cover - a type-only import; see below for why it is one.
    # This module is what ``hbd.bot.gate`` reaches into for ``COMMAND_PREFIX``, and it must
    # not be the module that first pulls SQLAlchemy into the middleware's own import chain.
    # The obvious justification — "the bot does not import ``hbd.db``" — is simply false:
    # ``handlers/commands.py`` imports ``hbd.db.retention`` at runtime and is imported by
    ...
    from hbd.db.retention import RetentionPolicy
...
async def reset_to_welcome(event: Event, state: FSMContext, deps: BotDeps) -> None:
    ...
    **The first screen is** :attr:`~hbd.bot.states.WizardStep.OCCASION`. The interface language
...
def privacy_text(language: Language, policy: RetentionPolicy) -> str:
    ...
    :class:`~hbd.db.retention.RetentionPolicy` changes this message in the same commit. The
