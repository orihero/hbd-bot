...
from hbd.bot.handlers.common import error_text, privacy_text, support_text
...
from hbd.contracts import Err, Language, Result, ok
...
async def handle_privacy(message: Message, state: FSMContext) -> None:
    ...
    **The body itself has moved to** :func:`~hbd.bot.handlers.common.privacy_text`, because
...
async def handle_forget(message: Message, state: FSMContext, deps: BotDeps) -> None:
    ...
    **The clear is BARE, and that is the change this release makes here.** Every other
    ...
    :mod:`hbd.db.purge` that is PLANNED and does not exist (``docs/product/ADMIN_PANEL_PLAN.md``
