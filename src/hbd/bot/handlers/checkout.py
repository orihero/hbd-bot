...
provider is :class:`~hbd.checkout.StubCheckoutProvider` by default — it contacts nothing,
opens no socket and reports every charge paid — and
:class:`~hbd.payme.provider.PaymeCheckoutProvider` when ``HBD_CHECKOUT_PROVIDER=payme``.
...
2. **The whole body runs inside ``try``/``finally``, and the finally puts the state back**
   ...
   :func:`~hbd.bot.handlers.common.expire` clears the session outright and
...
from hbd.bot.handlers.balance import show_balance, show_confirm
from hbd.bot.handlers.common import error_text, expire, present, read_draft, say
from hbd.bot.i18n import translate
from hbd.bot.middleware import resolve_language
from hbd.bot.screens import checkout_link_screen, menu_screen
from hbd.bot.states import Wizard
from hbd.checkout import Product, PurchaseRequest
...
# It lives on the write side, as ``hbd.db.purchases.CHECKOUT_ACTOR``, because
# ``hbd.checkout.PurchaseFulfiller`` carries no ``actor`` parameter at all — the bot cannot
