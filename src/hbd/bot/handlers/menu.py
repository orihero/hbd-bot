...
from hbd.bot.draft import UI_LANGUAGE_KEY
...
from hbd.bot.i18n import SUPPORTED_LANGUAGES, translate
from hbd.bot.keyboards import MENU_BUTTON_KEYS, MENU_LABELS
from hbd.bot.screens import menu_screen, settings_language_screen, settings_screen
...
async def handle_settings_language_chosen(
    ...
    **An ``Err`` from ``record_language`` is logged and swallowed**, because the two writes
    ...
    after ``hbd.runtime.jobs`` wipes the FSM dict on delivery, or after the fourteen-day TTL —
