...
from hbd.bot.handlers.common import Event, error_text, present, show_step
from hbd.bot.i18n import translate
from hbd.bot.keyboards import checkout_keyboard
...
def build_offer(balance: Result[CreditBalance] | None, deps: BotDeps) -> CheckoutOffer | None:
    ...
    * ``deps.purchases`` or ``deps.pricing`` is unset — this deployment does not sell.
      Every ``BotDeps`` built outside ``hbd.main`` wires neither, which is precisely what
    ...
    **This predicate deliberately does NOT consult ``EntitlementPolicy.is_balance_enforced``.**
    ...
    The two settings do belong together in production, which is why ``hbd.main`` warns at
