...
CATALOGUE: Final[dict[str, str]] = {
    ...
    # таклиф.
    #
    # ``{single_amount}`` ва ``{plan_amount}`` ``hbd.bot.pricing.format_amount`` орқали
    ...
    # ойнаси ҳамда ``HBD_PAYME_TRANSACTION_TIMEOUT_MS`` ва ``HBD_PAYME_INTENT_TTL_S`` учун
    ...
    # Атайлаб ``error.`` остида турмаган иккита рад жавоби: у префикс ``hbd.errors``га
    ...
    # атайлаб ``error.`` остида эмас: у префикс ``hbd.errors``га тегишли,
    ...
    # ёзиб қўйилган рақам бир куни ``HBD_SINGLE_SONG_PRICE_MINOR`` билан келишмай қолади —
    ...
    # аниқлаш калити.
    #
    # ``{handle}`` — бу ``hbd.watermark.WATERMARK_HANDLE``, муқова, ID3 теглари ва шу
