...
CATALOGUE: Final[dict[str, str]] = {
    ...
    # -- start -------------------------------------------------------------
    "start.welcome": (
        "🎂 Привет! Я собираю персональный поздравительный набор: песня и текст песни "
        "— с правильно произнесённым именем."
    ),
    ...
    "wizard.output_language.prompt": "На каком языке спеть песню?",
    "wizard.lyrics.writing": "✍️ Пишу текст песни… это займёт несколько секунд.",
    "wizard.lyrics.preview": (
        "<b>{title}</b>\n\n"
        "<pre>{lyrics}</pre>\n\n"
        "Нажмите ✅, чтобы оставить этот текст, 🔄 — чтобы получить другой, "
        "или просто пришлите сообщением свой текст."
    ),
    "wizard.lyrics.failed": "Не получилось написать текст песни. Попробуйте ещё раз через минуту.",
    ...
    "wizard.lyrics.updated": "Принято — возьму ваш текст.",
    "wizard.lyrics.too_many": (
        "Я уже написал {limit} вариантов текста для этой песни. "
        "Оставьте тот, что выше, или пришлите свой текст сообщением."
    ),
