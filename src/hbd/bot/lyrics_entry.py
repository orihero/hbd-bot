...
in ``hbd.watermark`` is added to rendered output — the preview screen's invite line, the
...
exactly ``"✨ Generate yours at @hbduzbot"`` — a voice singing the advertisement in the
middle of the birthday song, which is precisely the invariant ``hbd.watermark`` is shaped
...
from hbd.watermark import contains_watermark
...
def _without_watermark_lines(text: str) -> tuple[str, int]:
    ...
    THE DEFECT THIS FIXES: nothing filtered here, so the "✨ Generate yours at @hbduzbot"
    ...
    The predicate is :func:`hbd.watermark.contains_watermark` rather than a set of literals
    ...
    "✨ Сделайте свою в @hbduzbot", an Uzbek one "✨ Oʻzingiznikini @hbduzbot da yarating" —
    ...
    happen to name the bot — "thank you @hbduzbot for this song" — is dropped too. That is
    intended: there is no way to tell that line from a pasted one, and the invariant is
    about what gets sung, not about who typed it. A voice singing "at hbduzbot" ruins the
