...
``hbd.bot.screens`` a pure function of the draft — the thing that lets thirty-odd screen
...
silently disagree with ``HBD_SINGLE_SONG_PRICE_MINOR`` the day an operator changes it; a
...
class Pricing:
    ...
    def from_settings(cls, settings: Settings) -> Pricing:
        ...
        Importing ``hbd.config`` is fine here and is not fine in ``hbd.checkout``: this module
        lives under ``hbd.bot``, which is already the layer that owns composition-shaped
        values, while ``hbd.checkout`` is a leaf that ``hbd.db`` imports.
        """
        return cls(
...
class CheckoutOffer:
    ...
    This exists so ``hbd.bot.screens`` stays a pure function of the draft. Every field below
