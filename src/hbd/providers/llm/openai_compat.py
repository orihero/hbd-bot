...
* ``vendor`` and ``is_fallback`` are constructor facts, so the two accounts are finally
  ...
  dialect, not the billing relationship, and :class:`hbd.contracts.Vendor` is the axis an
  invoice arrives on.
...
* Every return path records exactly one :class:`hbd.usage.VendorUsage` — including the
  ...
  ``None`` third. Never zero: see ``hbd.providers.llm.pricing``.
"""
...
from hbd.errors import HbdError, ProviderRejectedContentError
...
def _status_from(error: HbdError | None) -> int | None:
...
class OpenAiCompatLlmProvider:
    ...
    async def _record_health(self, *, error: HbdError | None, latency_ms: int) -> None:
