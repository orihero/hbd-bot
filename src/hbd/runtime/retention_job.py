...
caller, and the hourly ``cron`` entry in :mod:`hbd.runtime.jobs` is the thing that fires it.
...
deleting bytes and then failing to commit (see :mod:`hbd.db.purge`). That design was
...
from hbd.db.purge import DEFAULT_PURGE_BATCH_SIZE, PurgeReport, purge_expired
...
async def _delete_objects(container: AppContainer, keys: tuple[str, ...]) -> StorageSweepResult:
    ...
    for key in keys:
        ...
        if is_err(removed):
            ...
            #
            # This is a DELIBERATE asymmetry with ``hbd.db.purge._purge``, which excludes
...
async def run_retention_sweep(
    ...
    *,
    ...
    differently (see :class:`hbd.db.enums.PurgeTrigger`).
