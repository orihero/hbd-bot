...
gateway is a fourth process (``hbd-payme.service``) and it holds exactly one credential — the
...
vendor key (:mod:`hbd.runtime.vendor_balance_job` makes the identical argument from the other
...
by ``HBD_PAYME_SWEEP_MINUTES``, not a silence.**

**The sweep is NOT gated on ``HBD_CHECKOUT_PROVIDER``, deliberately.** The documented rollback
...
the newly shared write primitive (:func:`hbd.db.fulfilment.write_single_sale`), which is now
...
``hbd.checkout.PaymentIntentOpener`` — a port with exactly one method, ``open_intent`` — which
...
The worker needs the WIDE port (:class:`hbd.payme.ports.PaymeLedger`). Widening the container
...
:mod:`hbd.runtime.vendor_balance_job` owns its ``httpx.AsyncClient`` instead of borrowing the
``ProviderSet``'s.
...
from hbd.bot.delivery import is_blocked_by_customer
from hbd.bot.i18n import parse_language, translate
from hbd.bot.keyboards import start_over_keyboard
from hbd.checkout import PaymentIntent, PaymentIntentState, PlanState, Product
from hbd.config import Settings
from hbd.contracts import Language, Result, is_err
...
from hbd.payme.ports import PaymeLedger, SettlementCounts
...
#: The kit job's context keys, RE-STATED rather than imported from :mod:`hbd.runtime.jobs`.
#: Importing them would make this module depend on the one that registers it, which is a
#: cycle; :mod:`hbd.runtime.retention_job` and :mod:`hbd.runtime.vendor_balance_job` restate
...
#: REGISTERED with in :mod:`hbd.runtime.jobs`, because ARQ compares ``job_try > max_tries`` at
...
#: with nobody told. ``hbd.runtime.jobs._is_final_attempt`` learned this the hard way for the
...
#: never the healthy path. A fuller audit is ``python -m hbd.payme.cli invariant``.
...
def payme_notify_job_id(public_ref: str) -> str:
    ...
    Deterministic for the reason ``hbd.pipeline.worker.job_id_for`` is: ARQ refuses to queue a
...
def build_worker_payme_ledger(container: AppContainer) -> PaymeLedger:
    ...
    ``hbd.checkout.PaymentIntentOpener``, and it is narrow on purpose. Returning
    :class:`hbd.payme.ports.PaymeLedger` rather than the concrete class is what makes
    ...
    ``HBD_PAYME_ENV_FILE`` variable, and this process deliberately does not load that file —
...
def _retry_or_give_up(ctx: Mapping[str, Any], *, public_ref: str, reason: str) -> None:
    ...
    ``hbd.runtime.jobs._is_final_attempt`` exists to prevent: ARQ discards that retry BEFORE
...
async def _send(
    ...
    **A ``Forbidden`` is stamped as delivered, and that is the interesting decision here.**
    ...
    :func:`hbd.bot.delivery.is_blocked_by_customer` so that a block and a deleted account are
...
async def _reconcile(container: AppContainer, ledger: PaymeLedger, *, now: datetime) -> int:
    ...
    * ``receipts > performed`` has one benign cause and it is worth knowing before paging
      ...
      ``python -m hbd.payme.cli journal --ref <ref>`` shows the ``settle_note`` that says so.
...
async def _name_the_orphans(ledger: PaymeLedger, *, frm: datetime, to: datetime) -> None:
    ...
    ``python -m hbd.payme.cli journal``, rather than three numbers and a database prompt.
    """
