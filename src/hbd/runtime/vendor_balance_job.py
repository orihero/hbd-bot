...
question the panel cannot answer. This job is the asker; :mod:`hbd.db.vendor_balances` is
the writer; ``hbd.db.admin.vendor_balances`` is the reader.
...
no new credential — see :mod:`hbd.runtime.vendor_balance_probes` on why OpenRouter's
...
day, and ``hbd.db.admin.vendor_usage._narrow`` filters only window and vendor — so health
...
from hbd.db.vendor_balances import PerSongRate, measure_per_song_rate, record_probe
from hbd.db.vendor_usage import DbUsageSink
from hbd.errors import PipelineError
from hbd.logging import get_logger
from hbd.providers.llm.openai_compat import DEFAULT_BASE_URL as OPENAI_DEFAULT_BASE_URL
...
def _fallback_base_url(settings: Settings) -> str:
    ...
    Mirrors ``hbd.providers.llm.factory.build_fallback_llm_provider`` exactly: an unset
    ...
    if settings.llm_api_key and is_openrouter(settings.llm_base_url):
        targets.append(
            _Target(
                ...
                # cautious one. ``HBD_OPENROUTER_MANAGEMENT_KEY`` names one account's
