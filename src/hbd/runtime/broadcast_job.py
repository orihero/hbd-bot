...
* **claim, commit, then send.** :meth:`~hbd.db.broadcasts.SqlBroadcasts.claim` moves a row to
  ``sending`` in its own committed transaction *before* the Telegram call is made, and
  :meth:`~hbd.db.broadcasts.SqlBroadcasts.settle` closes it after. A job killed in between
...
:func:`~hbd.db.broadcasts.release_recipient`, and it may be used **only** when the request
...
``UPDATE`` in :func:`~hbd.db.broadcasts.claim_chunk` is, and it holds however many chunk jobs
happen to be alive.

The registration lives in :mod:`hbd.runtime.jobs` with every other job, and the cron entry
...
from hbd.admin.schemas.segment import SegmentModel, to_segment
from hbd.bot.delivery import is_blocked_by_customer
from hbd.bot.i18n import FALLBACK_LANGUAGE
from hbd.bot.locales import REFERENCE_LANGUAGE
from hbd.config import ENV_PREFIX, env_file
...
from hbd.db.admin.segment import compile_segment, segment_capabilities
...
#: PROCESS — the admin panel, through :mod:`hbd.admin.queue`, which restates these same three
...
#: :func:`hbd.runtime.payme_jobs.sweep_minutes` gives at length — a set is unhashable and
...
#: closed to us. Matched the way :func:`hbd.bot.delivery.is_blocked_by_customer` matches its
...
#: The placeholder ``hbd.admin.audit_sink`` writes when a value is refused at the boundary.
...
class _AuditChainSettings(BaseSettings):
    """The audit chain key, read on its own rather than through :class:`hbd.config.Settings`.
    ...
    **It cannot be a field on ``Settings`` and this is not a workaround.**
    ...
    make ``HBD_ADMIN_AUDIT_HMAC_KEY`` a variable the *panel* is refused at boot, which is the
    one process that cannot run without it. A separate settings model is the shape
    :class:`hbd.payme.settings.PaymeSettings` already uses for a secret that must not live on
    the shared object.
...
def _store(container: AppContainer) -> SqlBroadcasts:
    ...
    :func:`hbd.runtime.payme_jobs.build_worker_payme_ledger`'s precedent: the container holds
...
async def _enqueue(
    ...
    :func:`hbd.runtime.payme_jobs.run_payme_sweep`'s delivery arm treats it, rather than
...
def _compile(plan: BroadcastPlan) -> Result[ColumnElement[bool] | None]:
    ...
    the one the operator previewed. The import costs no FastAPI — ``hbd.admin.schemas`` is
...
class _Chunk:
    ...
    def language_of(self, recipient: ClaimedRecipient) -> ComposedBody | None:
        ...
        Exact language, then ``uz_latn``, then ``en`` — ``hbd.bot.i18n._resolve_template``'s
...
async def send_broadcast_chunk(ctx: Mapping[str, Any], broadcast_id: str) -> dict[str, Any]:
    ...
    3. up to :data:`~hbd.db.broadcasts.SEND_CHUNK_SIZE` rows are claimed, each by a
...
async def _append_outcome(
    ...
    The retry mirrors ``hbd.admin.audit_sink._append``: a value refused at the boundary is an
