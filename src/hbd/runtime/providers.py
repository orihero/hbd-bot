...
``build_provider_set`` takes a :class:`~hbd.usage.UsageSink` and threads the SAME object
...
The parameter DEFAULTS to :data:`~hbd.usage.LOGGING_USAGE_SINK` because a caller with no
database — a test, a one-shot operator tool, ``hbd.demo`` — must still get a correct
provider set and a log line for each call; ``hbd.runtime.container`` is what substitutes
...
from hbd.usage import LOGGING_USAGE_SINK, UsageSink
