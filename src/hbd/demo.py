"""One celebration kit, start to finish, with no keys and no network. ``python -m hbd.demo``.
...
from hbd.logging import configure_logging, get_logger, new_correlation_id
from hbd.names import resolve_name
from hbd.pipeline.events import ProgressEvent, ProgressStatus
...
def _build_order(settings: Settings) -> Order | HbdError:
...
def _report_name(order: Order) -> None:
    ...
    _say("  candidates (rank order comes from HBD_NAME_CANDIDATE_ORDER):")
...
def main() -> int:
    """``python -m hbd.demo [output-directory]``. Never raises."""
    ...
    with tempfile.TemporaryDirectory(prefix="hbd-demo-") as scratch:
