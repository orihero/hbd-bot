...
``HBD_PAYME_MERCHANT_KEY``, which lives on ``hbd.payme.settings.PaymeSettings`` and is
...
#: what it has always been, so a checkout with a ``.env`` and no ``HBD_ENV_FILE`` behaves
#: exactly as before.
ENV_FILE: Final[str] = ".env"

#: The variable that selects a different one — ``HBD_ENV_FILE=.env.prod``, which is how
#: ``ENV=prod make dev`` exercises a production configuration from a development machine, and
#: how a systemd unit points at ``/etc/hbd/bot.env`` outside the checkout.
#:
#: **Read from the process environment ONLY**, and it has to be: a dotenv file cannot name
#: the dotenv file that is about to be read. Writing ``HBD_ENV_FILE`` *into* ``.env`` is
...
#: typo is a boot failure naming ``HBD_CHECKOUT_PROVIDER`` instead of a silent fall-through to
...
#: — ``hbd.checkout.STUB_PROVIDER_NAME`` and ``hbd.payme.ports.PAYME_PROVIDER_NAME``, which is
#: what the composition root compares against and what the ledger writes onto every receipt. A
#: ``Literal`` cannot be built from imported names, and this module must not import either
#: package anyway (``hbd.checkout`` is a leaf ``hbd.db`` depends on; ``hbd.payme`` is a rail).
#: ``tests/test_runtime/test_wiring.py`` asserts the three spellings agree, so the duplication
#: is pinned rather than merely noticed.
...
#: process — ``hbd.payme.settings.PaymeSettings`` — which the admin host must still never
#: hold, and which the BOT must never hold either.
#:
#: ``HBD_PAYME_MERCHANT_KEY`` is the Payme cashbox key: an INBOUND VERIFICATION secret, whose
...
#: admin lifespan, and ``hbd.main``'s prod boot check — read both.
...
class Settings(BaseSettings):
    ...
    vendor_balance_poll_enabled: bool = Field(
        ...
        description=(
            ...
            "HBD_USE_FAKE_PROVIDERS: that says 'contact no vendor at all', this says 'do not "
    ...
    #: ``hbd.bot.pricing.format_amount`` divides back down for display; nothing else does.
    ...
    #: cannot import :mod:`hbd.config` at all, and it used to be listed here as a caller that
    ...
    #: (:mod:`hbd.admin.settings`) and must be moved with it.
    #:
    #: 0 is a valid policy and not a degenerate one: ``EntitlementPolicy.__post_init__``
    #: refuses only negatives, and 0 makes ``hbd.db.credits._mint_due_allowance`` return
    ...
    #: fan-out that shares the same Redis bucket by name (``hbd.runtime.pacer``). The cost
    ...
    broadcast_sending_lease_s: float = Field(
        ...
        description=(
            ...
            "'unknown'. Must exceed HBD_BROADCAST_CHUNK_TIMEOUT_S."
        ),
    )
    ...
    #:
    #: A ``Literal`` and not a ``str``, so a typo is a named boot failure — ``HBD_CHECKOUT_
    ...
    checkout_provider: CheckoutRail = Field(
        default="stub",
        description="stub | payme. Flip to payme WITH credits_enforced; see hbd.main's refusal.",
    )
    ...
    #: ``hbd.runtime.container._build_checkout``, which is the ``build_llm_provider``
    ...
    #: second half is a promise the rail will not keep. **The bound that actually matters is
    #: not expressible here** — this must be strictly less than the wizard-state TTL, or a
    #: customer could pay for a draft that has already expired — because that number lives in
    #: ``hbd.bot.app``, which imports this module. ``hbd.main`` asserts it at boot instead, and
    #: the two checks are not redundant: this one is about the rail, that one is about two
    #: subsystems' clocks agreeing.
...
def build_settings(
    ...
    *,
    ...
    The dotenv file is :func:`env_file` — ``.env`` unless ``HBD_ENV_FILE`` names another —
