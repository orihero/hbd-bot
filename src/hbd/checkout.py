"""The seam for BUYING — distinct from ``hbd.payments``, which authorises a RENDER.

Two questions look like one and are not. ``hbd.payments.PaymentProvider.authorize`` answers
...
pressed. That row's shape lives HERE rather than in ``hbd.db`` or in a rail package for the
...
This module sits at the same depth as :mod:`hbd.entitlements` and imports only leaves —
``hbd.contracts``, ``hbd.logging`` (which itself imports nothing but ``hbd.errors``) and
``hbd.entitlements``, for the ``CreditBalance`` a fulfilled single song hands back. **It may
never import ``hbd.db``.** The persistence side implements :class:`PurchaseFulfiller` from
the other direction, exactly the way ``hbd.db.credits.SqlCreditLedger`` implements
``EntitlementStore`` — which is what keeps ``hbd.db`` free to import this module without the
cycle that putting these types in ``hbd.contracts`` reproduced for the entitlement types.
"""
...
from hbd.contracts import Result, ok
...
class Plan(StrEnum):
    ...
    layer stores these in, which is what lets ``hbd.db.enums.PlanKind`` mirror it as a plain
...
class Purchase(BaseModel):
    ...
    #:
    #: **This field has a reader now.** ``hbd.bot.handlers.checkout._settle`` branches on it:
...
class PlanStatus(StrEnum):
    ...
    **``LAPSED`` is not "did not renew".** There is no renewal in this product: no
    ...
    it lives beside :class:`PlanState` rather than in ``hbd.db.enums``.
    """
...
class PurchaseFulfiller(Protocol):
    ...
    **``fulfil_single`` now writes TWO rows, and the invariant survives that.** A sale is
    ...
    song with no record of the money. ``hbd.db.purchases`` states the same thing from the
    implementation side.
...
# in this module — the leaf that may never import ``hbd.db`` — precisely so that persistence
...
class PaymentIntent:
    ...
    The frozen view that crosses the ``hbd.db`` boundary: a ``*Row`` never leaves that package
