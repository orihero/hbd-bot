...
may bill (``src/hbd/bot/handlers/lyrics.py:80``), and ``handlers.common.reset_to_welcome``
...
**Why this is not ``hbd.ratelimit``.** That module's counters are an in-process dict, which
...
``hbd.ratelimit``'s rule too and for the same reason; see :class:`LyricBudgetStore`.
...
``hbd.bot.deps.BotDeps.entitlements`` literally read-only for gates: the bot writes a lyric
counter, never a credit.

**This module is a leaf.** It imports ``hbd.contracts``, ``hbd.errors`` and
``hbd.entitlements`` — itself a leaf — and nothing else. It may never import ``hbd.config``
(see :func:`resolve_lyric_budget_policy`, which reads ``Settings`` defensively exactly as
``hbd.entitlements`` and ``hbd.ratelimit`` do) and never ``hbd.db``.
...
:func:`hbd.entitlements.period_index_for` is what stops two different "which window is it?"
...
from hbd.entitlements import period_index_for, period_start
...
class LyricBudgetPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        Mirrors :meth:`hbd.entitlements.EntitlementPolicy.__post_init__` and
        :meth:`hbd.ratelimit.InboundPolicy.__post_init__` so this codebase has one idiom for
...
def resolve_lyric_budget_policy(settings: object | None = None) -> LyricBudgetPolicy:
    ...
    the reason the module docstring gives: importing ``hbd.config`` would give this leaf the
    one edge it has none of. The third instance of an idiom this repo already runs twice
    (``hbd.entitlements.resolve_entitlement_policy``, ``hbd.ratelimit.resolve_inbound_policy``).
    """
    if settings is None:
...
def day_resets_at(index: int) -> datetime:
    ...
    end. Same rule as ``hbd.entitlements.period_start``, one window along.
    """
...
class LyricBudgetVerdict:
    ...
    :class:`hbd.ratelimit.ThrottleVerdict` reports its count: it is the only thing that tells
...
class LyricBudgetStore(Protocol):
    ...
    ``hbd.db.lyric_budget.SqlLyricBudget``.
    ...
    async def claim_lyric_write(
        ...
        on forever, and it is the same rule :func:`hbd.ratelimit.check_update_rate` states.
