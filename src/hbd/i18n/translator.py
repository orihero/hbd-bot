...
Strictness is decided once, at translator construction: ``HBD_I18N_STRICT`` wins if it
...
from hbd.errors import GENERIC_USER_MESSAGE_KEY, HbdError, ValidationError
from hbd.i18n.catalog import Catalog, Entry, load_catalogs
from hbd.i18n.plurals import plural_category
...
STRICT_ENV_VAR: Final[str] = "HBD_I18N_STRICT"
...
class Translator:
    ...
    def error_message(self, error: HbdError, language: Language) -> str:
        """The customer-facing side of an ``HbdError``.
...
def translate_error(error: HbdError, language: Language) -> str:
