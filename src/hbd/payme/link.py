...
pooled ``httpx.AsyncClient``; this one has no counterpart, and ``hbd.payme.provider`` carries a
...
1. **A ``;`` anywhere inside a VALUE silently truncates that value.** There is no escaping
   ...
   (``HBD_PAYME_RETURN_URL``, WS-G) rather than here. This function is total and cannot fail,
...
#: code, because that is the form ``hbd.checkout.Purchase.currency`` and every receipt column
...
#: language added to ``hbd.contracts.Language`` must be answered for here, and a ``KeyError``
...
def build_checkout_link(
    *,
    ...
    if return_url:
        ...
        # silently, which is why ``HBD_PAYME_RETURN_URL`` refuses one at settings-build time.
