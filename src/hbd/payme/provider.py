...
UNPAID :class:`~hbd.checkout.Purchase` carrying a ``checkout_url``. Nothing is granted. Some
...
the SAME ``public_ref`` for a replay. Because :func:`~hbd.payme.link.build_checkout_link` is
...
**Layering.** This module is in ``hbd.payme``, so it may never import ``hbd.db`` (see this
package's ``__init__``). It receives a :class:`~hbd.checkout.PaymentIntentOpener` — the
...
from hbd.checkout import PaymentIntentOpener, Product, Purchase, PurchaseRequest
from hbd.contracts import Language, Result, err, is_err, ok
from hbd.errors import CheckoutPausedError
from hbd.logging import get_logger
from hbd.payme.link import build_checkout_link
...
class PaymeCheckoutProvider:
    ...
    Structurally satisfies :class:`~hbd.checkout.CheckoutProvider` and deliberately does NOT
    ...
    **Everything it needs is injected and nothing is read from ambient settings**, so a test
    ...
    async def charge(self, request: PurchaseRequest) -> Result[Purchase]:
        ...
        **Nothing in this method can raise, and that is established rather than hoped for.**
        ...
        ``run_guarded``; :func:`~hbd.payme.link.build_checkout_link` is pure and total;
