...
endpoint is not a router on the admin app: ``hbd.admin.errors.install_error_handlers`` is
...
``hbd/admin/app.py`` derives ``FORBIDDEN_ENV_VARS`` from a tuple this integration widens with
the merchant key, so a prod admin process would refuse to boot once the key was in reach;
``HBD_ADMIN_ENABLED=false`` is the documented incident kill switch and refusing operator logins
...
``hbd-bot.service``'s own header is "Long polling, so it needs outbound HTTPS and nothing
...
6. Everything else is :class:`hbd.payme.service.PaymeService`'s, including the unknown-method
   ``-32601``.
...
**``hbd.runtime.startup.verify_host`` is deliberately NOT called.** It checks ffmpeg, the
...
**``hbd.config.get_settings()`` is never called either.** That function is an
...
``app`` is built at import so ``uvicorn hbd.payme.app:app`` works, and :func:`create_app`
therefore reads no configuration: settings are built inside the lifespan. Importing this module
must never fail on a missing environment variable, or a typo in ``/etc/hbd/payme.env`` becomes
...
from hbd.config import ENV_PREFIX, VENDOR_SECRET_FIELDS, resolve_env_file
from hbd.db.engine import ping
from hbd.errors import ConfigError
from hbd.logging import configure_logging, get_logger
from hbd.payme.auth import presented_login, verify_basic
from hbd.payme.container import PaymeContainer, build_payme_container
from hbd.payme.protocol import PaymeErrorCode, RpcRequest, render_fault
...
#: Constant rather than merely short, for ``hbd.admin.routers.health``'s reason: "degraded" is
...
#: ``HBD_LLM_FALLBACK_API_KEY`` — the credentials that belong to ``hbd.config.Settings`` and
#: that THIS host must never hold either. Derived from ``hbd.config`` rather than restated, so
#: a fifth vendor credential added there is covered here without anyone remembering this file
#: exists. It is the mirror image of ``hbd.admin.app.FORBIDDEN_ENV_VARS`` and derives from the
...
#:
#: ``HBD_PAYME_MERCHANT_KEY`` is of course NOT on this list. This is the process that holds it;
...
_DISABLED_MESSAGE: Final[str] = (
    "The Payme gateway is disabled. Set HBD_PAYME_ENABLED=true in the file named by "
    "HBD_PAYME_ENV_FILE to run it."
)


# ---------------------------------------------------------------------------
# The boot-time credential scan — the mirror image of hbd.admin.app's
# ---------------------------------------------------------------------------
def _payme_env_file() -> str:
    """The dotenv this process actually reads — ``.env.payme`` or ``HBD_PAYME_ENV_FILE``.
    ...
    so a test that monkeypatches :data:`hbd.payme.settings.PAYME_ENV_FILE` redirects both.
    """
...
def _env_file_names() -> frozenset[str]:
    ...
    ``hbd_telegram_bot_token`` line is just as live as the shouted form.
    """
...
def _refuse_foreign_credentials(settings: PaymeSettings) -> None:
    ...
    In dev it is a WARNING for ``hbd.admin.app``'s reason: a shared developer ``.env`` is
...
def _refuse_a_blank_key(settings: PaymeSettings) -> None:
    ...
    ``hbd-payme.service``'s ``InaccessiblePaths`` makes against the application-level check
    it duplicates.
    """
...
def _normalised_ip(value: str | None) -> str | None:
    ...
    ``hbd.admin.security.clientip._parse_ip`` applies, restated in a dozen lines because
    :mod:`hbd.payme` may not import :mod:`hbd.admin`: they are different processes with
...
#: 10 000-entry header does. Matches ``hbd.admin.security.clientip.MAX_FORWARDED_ENTRIES``.
...
def _refuse_unknown_caller(peer: str | None, settings: PaymeSettings) -> bool:
    ...
    :attr:`hbd.payme.settings.PaymeSettings.allowed_networks` is empty unless an operator has
...
class JsonRpcErrorMiddleware:
    ...
    **Ours, and never ``hbd.admin.errors.install_error_handlers``.** That one is application
    ...
    ``hbd.admin.middleware.unhandled`` documents: Starlette routes that key to
...
def _register_routes(application: FastAPI) -> None:
    ...
    async def readyz(request: Request) -> dict[str, Any]:
        ...
        Gated for ``hbd.admin.routers.health``'s reason: an unauthenticated detailed body is
    ...
    async def lifespan(application: FastAPI) -> AsyncIterator[None]:
        ...
        _LOGGER.info(
            ...
            extra={
                ...
                # HBD_PAYME_ENV_FILE can boot a production gateway from a dev checkout.
...
#: The uvicorn entry point (``make payme``, ``hbd-payme.service``). Constructing it reads
