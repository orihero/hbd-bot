"""``python -m hbd.payme.harness`` — Payme's own sandbox scripts, replayed against a live gateway.
...
``make test``, and by this script over a real socket against the real ``hbd-payme.service`` on
...
**This module imports ``hbd.payme.container``, which is the package's one sanctioned door into
``hbd.db``.** It needs a database because opening an intent is the BOT's port and there is no
bot in the room; it goes through the composition root rather than reaching for ``hbd.db``
...
from hbd.contracts import is_err
from hbd.errors import HbdError
from hbd.logging import configure_logging, get_logger
from hbd.payme.container import PaymeContainer, payme_container
from hbd.payme.protocol import CancelReason, PaymeErrorCode, PaymeMethod, to_ms
from hbd.payme.settings import PaymeSettings, build_payme_settings
...
#: in a web form, and a mismatch with ``HBD_PAYME_ACCOUNT_FIELD`` turns every
...
#: ``HBD_PAYME_DUPLICATE_TRANSACTION_CODE`` as the gateway under test is actually configured.
...
class Step:
    ...
    ``method`` is a plain ``str`` and not :class:`hbd.payme.protocol.PaymeMethod`, for one row's
...
class Credentials:
    ...
    def header(self, auth: Auth) -> str | None:
        ...
        mistyped key in ``/etc/hbd/payme.env`` actually has.
        """
...
# live gateway), following ``hbd.tools.credits`` — the repository's other operator CLI and the
...
class RefusedError(Exception):
    ...
    An exception rather than a ``Result``, for the reason ``hbd.tools.credits.RefusedError``
...
def _parse(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m hbd.payme.harness",
        description=(
            "Replay Payme's published sandbox scripts against a running hbd-payme gateway. "
    ...
    parser.add_argument(
        "--login",
        default="",
        help="Basic-auth login (default: HBD_PAYME_BASIC_LOGIN, itself defaulting to Paycom)",
    )
    parser.add_argument(
        "--key",
        default="",
        help=(
            "the merchant key the gateway is running with (default: HBD_PAYME_MERCHANT_KEY). "
...
def plan(argv: Sequence[str], *, settings: PaymeSettings) -> Request:
    ...
    operator who has them in ``/etc/hbd/payme.env`` should not have to retype a secret onto a
    ...
    if not credentials.key:
        raise RefusedError(
            "no merchant key: set HBD_PAYME_MERCHANT_KEY in the gateway's dotenv or pass --key. "
...
async def apply(request: Request, *, settings: PaymeSettings) -> list[ScenarioOutcome]:
    ...
    The container is this script's own — built by ``hbd.payme.container.payme_container``, the
