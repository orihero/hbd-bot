...
5. **Errors in the ``-31050..-31099`` range MUST carry ``data`` set to the account subfield
   ...
   deliberately NOT taken from ``hbd.bot.i18n``: that catalogue has FOUR languages because
...
``hbd.db.enums.PlanKind`` and ``hbd.checkout.Plan`` are separate: a column and a wire format
...
number on the other. :func:`hbd.payme.rules.wire_state` is the single crossing point.

``PaymeState`` is also mirrored, value for value, by ``hbd.db.enums.PaymeState``, which is
...
#: so it is a setting (``HBD_PAYME_ACCOUNT_FIELD``) whose default lives here beside the codes
...
class PaymeErrorCode(IntEnum):
    ...
    **``-31054`` is deliberately unallocated.** Payme's own materials contradict each other on
    ...
    emitted through the ``HBD_PAYME_DUPLICATE_TRANSACTION_CODE`` setting, defaulting to
...
class PaymeState(StrEnum):
    ...
    ``hbd.db.enums.PaymeState``, which is what the mapped column actually uses — this package
    ...
    ``hbd.checkout.Plan`` and ``hbd.db.enums.PlanKind`` already take.

    Longest value is ``cancelled_after_perform`` at 23 characters, inside the 32-character
    ``ENUM_LENGTH`` that ``hbd.db.base.enum_type`` allocates. That is asserted on the
...
def to_ms(moment: datetime) -> int:
    ...
    **A NAIVE datetime is interpreted as UTC rather than refused.** Every clock this system
    owns is ``hbd.db.base.utc_now()``, which is aware, and ``UtcDateTime`` raises on a naive
...
def from_ms(value: int) -> datetime:
    ...
    ``payme_transactions.payme_time``, and ``hbd.db.base.UtcDateTime`` raises on a naive bind
...
def localised(ru: str, uz: str, en: str) -> dict[str, str]:
    ...
    Three keys, and exactly three. ``hbd.bot.i18n`` carries four languages because Uzbek is
