"""``python -m hbd.payme.cli`` — the rail's recovery surface, and on day one the ONLY one.
...
**The rule this file inherits from :mod:`hbd.tools`, and where it consciously departs.**
``hbd.tools``' own docstring states it: an operator tool WRITES through the same functions the
...
:func:`hbd.payme.pause.set_paused`. **The departure is on the READ side and it is deliberate.**
...
together and should not: :class:`hbd.payme.ports.PaymeLedger` is the surface an INTERNET-FACING
...
**This is the SECOND module in ``hbd.payme`` that imports ``hbd.db``**, after
:mod:`hbd.payme.container`, whose docstring calls itself the only one. That sentence needs
...
other file here would still typecheck with ``hbd.db`` deleted from the tree, and so would the
gateway. Nothing imports this module; it has no callers inside the product at all, which is
``hbd.tools``' own rule for an operator tool and the reason it can afford the wider reach.
...
from ``HBD_PAYME_ENV_FILE`` — so it must be run on the host that holds ``/etc/hbd/payme.env``.
...
from hbd.checkout import PaymentIntent, PaymentIntentState
from hbd.contracts import Result, is_err
from hbd.db.enums import PaymentIntentState as StoredIntentState
from hbd.db.enums import PaymeState as StoredPaymeState
...
from hbd.db.models.payme_transaction import PAYME_TRANSACTION_ID_LENGTH, PaymeTransactionRow
from hbd.db.models.payment_intent import PUBLIC_REF_LENGTH, PaymentIntentRow
...
from hbd.payme.container import PaymeContainer, QueuedNotifier, payme_container
from hbd.payme.pause import PauseSwitch, is_paused, set_paused
from hbd.payme.ports import PaymeLedger, PaymeStatementRow
from hbd.payme.settings import build_payme_settings
...
#: What ``payment_intents.settle_note`` starts with when a human settled the intent rather than
#: a ``PerformTransaction``. It is written by ``hbd.db.payme.SqlPaymeLedger._force_settle`` and
#: only READ here, to split the settlement counts into the two populations an operator has to
#: tell apart. It is spelled again rather than imported because the ledger keeps it private, and
#: an underscore import would be a promise about another module's internals; the coupling is
#: instead pinned by ``test_cli.py``, which asserts a real force-settled row starts with it.
OPERATOR_SETTLE_PREFIX: Final[str] = "operator:"

#: How long a settled intent is left alone before ``reconcile`` re-enqueues its announcement.
#: The same grace ``hbd.runtime.payme_jobs`` uses, and for the same reason: an intent settled
#: two seconds ago probably has a job in flight, and racing it would be two messages about one
#: song. Restated rather than imported because ``hbd.payme`` may not depend on ``hbd.runtime``.
...
class RefusedError(Exception):
    ...
    An exception rather than a ``Result``, the same shape :mod:`hbd.tools.credits` and
    :mod:`hbd.admin.bootstrap` use and for the same reason: :func:`main` is the boundary of the
...
class OperatorConsole:
    ...
    A value object rather than the whole :class:`hbd.payme.container.PaymeContainer` for two
...
def console_for(container: PaymeContainer) -> OperatorConsole:
    ...
    The notifier is the container's OWN :class:`hbd.payme.container.QueuedNotifier`, deliberately
...
def plan(argv: Sequence[str]) -> Request:
    ...
    Separated from :func:`main` for the reason :func:`hbd.tools.credits.plan` is: every refusal
...
def _bounded(raw: str, flag: str, *, low: int, high: int) -> int:
    ...
    judgement :mod:`hbd.tools.credits` makes about a Telegram id.
    """
    text = str(raw).strip()
...
async def apply(console: OperatorConsole, request: Request) -> Report:
    ...
    Takes the console rather than a session, matching :func:`hbd.tools.credits.apply`: every
...
async def _reconcile(console: OperatorConsole, request: Reconcile) -> Report:
    ...
    The same three arms ``hbd.runtime.payme_jobs.run_payme_sweep`` runs on its cron, in the same
