...
``hbd.admin.security.ratelimit`` already runs on the admin login route, and the duplication
...
**Everything here is a leaf.** It imports ``hbd.errors``, ``hbd.logging`` and the standard
library, and nothing else — not ``hbd.config`` (see :func:`resolve_inbound_policy`, which
reads a ``Settings`` defensively through ``getattr`` exactly as ``hbd.entitlements`` does),
not ``aiogram``. The aiogram half lives in ``hbd.bot.gate``.
...
#: inbound update, taken inside the FSM isolation lock — see ``hbd.bot.gate``.
...
#: which live under ``hbd:admin:rl``.
_KEY_PREFIX: Final[str] = "hbd:bot:rl"
...
class InboundPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        :meth:`hbd.entitlements.EntitlementPolicy.__post_init__` so the codebase has one
...
def resolve_inbound_policy(settings: object | None = None) -> InboundPolicy:
    ...
    the same reason :func:`hbd.entitlements.resolve_entitlement_policy` does it: this module
    is a leaf and importing ``hbd.config`` would give it the one edge it has none of.
    """
    if settings is None:
