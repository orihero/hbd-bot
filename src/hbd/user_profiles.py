...
**This module is a leaf and must stay one.** It imports ``hbd.contracts`` (and would import
``hbd.errors`` if it ever named a refusal of its own) and NOTHING else: never ``hbd.db``,
never ``hbd.config``. ``hbd.entitlements`` and ``hbd.lyric_budget`` both state that rule in
...
protocol, and not on SQLAlchemy — ``hbd.db.user_profiles.SqlUserProfiles`` is the only thing
in the tree that knows a ``user_profiles`` row exists, and the ``hbd.db`` package imports
*this* module rather than the reverse, so there is no back-edge for a circular import to come
back through. (``hbd.entitlements`` records what that cycle cost when the same types were
tried in ``hbd.contracts``.)
...
#:
#: ``hbd.bot.avatar`` imports this rather than declaring its own, and
#: ``hbd.admin.routers.users`` builds ``AVATAR_MIMES = frozenset({AVATAR_MIME})`` from it —
#: legal, because ``tests/test_admin/test_asset_stream.py:712`` caps only what the admin
#: package imports from ``hbd.storage``. Three literals in three layers would drift silently
...
def avatar_key(user_id: object) -> str:
    ...
    1. **It lives here and not in ``hbd.storage``.** ``tests/test_admin/test_asset_stream.py``
       (line 712) asserts that everything the admin package imports from ``hbd.storage`` is
       ...
       ``hbd.storage`` would fail that assertion the day the route lands, and the cheap way
       out of a failing import cap is to widen the cap — which is the whole point of it gone.
       ``hbd.storage`` gets no edit at all.
...
class UserProfileStore(Protocol):
    ...
    Every method returns a ``Result`` and NONE of them raises — the rule ``hbd.contracts``
    states for every protocol in this codebase, honoured on the other side by
    ``hbd.db.guard.run_guarded``, which turns a ``SQLAlchemyError`` into an ``Err``. Onboarding
    ...
    The implementation is ``hbd.db.user_profiles.SqlUserProfiles``, which holds the
