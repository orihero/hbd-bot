...
``to`` to :class:`~hbd.db.admin.audit.AuditQuery` as two independent bounds rather than as a
...
* ``?to=Y`` alone means "everything up to Y". Its missing end is not invented at all — it is
  *absent*, and :class:`~hbd.db.admin.sql.TimeWindow` now models that as ``start is None`` so
...
:func:`~hbd.db.admin.sql.time_window`, which owns that rule for every caller including the
...
**Not in** :mod:`hbd.admin.schemas.window`: there is no wire model here, and ``schemas/`` is
where camelCase pydantic lives. **Not in** :mod:`hbd.admin.deps`: that module is the
...
:mod:`hbd.db.admin.sql`: that layer owns the SQL type and its ordering rule, and it must not
