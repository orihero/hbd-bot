...
:data:`~hbd.pipeline.events.STAGE_ORDER` has to be **reconstructed** from the attempts it
...
``RENDERING_GREETINGS`` entirely (:func:`~hbd.pipeline.events.scheduled_stages`), so a
...
from hbd.contracts import AssetKind, OrderState
from hbd.db.admin.views import AssetView, AttemptView, OrderListItem
from hbd.db.enums import GenerationKind
from hbd.pipeline.events import GREETING_STAGES, STAGE_ORDER, PipelineStage
...
class StageStatus:
    ...
    error_code: str | None
    #: ``None`` when no class in ``hbd.errors`` claims the code — see
    #: :mod:`hbd.admin.serializers.retryability`.
    is_retryable: bool | None
    #: True for the two stages a song-only kit does not run, so the SPA can ghost them
    #: without restating :data:`~hbd.pipeline.events.GREETING_STAGES`.
