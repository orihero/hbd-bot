...
module differs from ``/users`` on.** :class:`~hbd.admin.schemas.users.UserView` publishes the
...
from hbd.admin.schemas.segment import SegmentModel
from hbd.admin.serializers.redaction import mask_telegram_user_id
from hbd.bot.i18n import escape_html
...
# ``hbd.admin.errors._handle_validation`` states: a pydantic message is built from the
...
def render_body_html(text: str) -> str:
    ...
    Literal runs are escaped with the bot's own :func:`~hbd.bot.i18n.escape_html`; allowlisted
...
class BroadcastCreateRequest(ApiModel):
    ...
    kind: BroadcastKind
    #: Stored on the row verbatim, never re-evaluated. See ``hbd.db.models.broadcast``.
...
class BroadcastSendRequest(ReasonedRequest):
    ...
    **Naive is refused here; "in the past" is not.** A missing UTC offset is a property of the
    ...
    ``hbd.admin.window.require_aware``'s own words because an operator should not meet two
...
class BroadcastDetailView(ApiModel):
    ...
    **``segment`` is nullable and its readability is a field, because a stored document is
    ...
    ``hbd.db.admin.views`` takes towards a purge — and ``segmentHash`` still identifies it.
    """
...
def to_body_view(record: BroadcastBodyRecord) -> BroadcastBodyView:
    ...
    :func:`~hbd.db.admin.broadcasts.broadcast_body_view`, one layer below, so the live handle
...
def to_broadcast_detail_view(detail: BroadcastDetail) -> BroadcastDetailView:
    ...
    :class:`~hbd.db.admin.views.BroadcastDetail` is assembled by one function from one campaign
