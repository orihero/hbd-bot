...
``hbd.admin.schemas.audit`` deliberately does **not** use this: the audit log pages on ``seq``,
...
:data:`~hbd.db.admin.page.TOTAL_COUNT_CAP`, so a bare ``total`` of 10 000 would be read as a
...
def page_params(
    ...
    parameter, and :func:`~hbd.db.admin.page.page_request` refuses it again for every caller
...
#: wiring — same split as ``Db`` and ``Container`` in :mod:`hbd.admin.deps`.
