...
**This module is the boundary, not the meaning.** :mod:`hbd.db.admin.segment` owns every
...
* **Meaning** — an unknown field, an operator that field does not take, a value the field's
  ...
  :func:`~hbd.db.admin.segment.compile_segment` refuses these, there.
...
before :mod:`hbd.admin.window` deleted them.
...
fields are :attr:`~hbd.db.admin.segment.FieldKind.INSTANT` and parses those values into
...
:class:`~hbd.admin.schemas.common.ApiModel` is what makes a hybrid a 422 rather than a
...
:class:`~hbd.admin.schemas.users.UserView` carries the raw Telegram id by design — every
...
:func:`~hbd.db.admin.page.decode_cursor` takes for the same reason, and its refusals carry
...
from hbd.db.admin.views import SegmentBreakdown
...
#: largest document :data:`~hbd.db.admin.segment.SEGMENT_LIMITS` permits, and small enough
...
#: :data:`~hbd.db.admin.segment.SEGMENT_REFUSALS` names the columns that would need one.
...
class RuleModel(ApiModel):
    ...
    registry is the allowlist and :func:`~hbd.db.admin.segment.compile_segment` is where a
...
class SortModel(ApiModel):
    ...
    The DIRECTION is settled by the type — :data:`~hbd.db.admin.segment.SortDirection` is a
...
class SegmentModel(ApiModel):
    ...
    rules: tuple[SegmentNodeModel, ...] = ()
    #: Absent means :data:`~hbd.db.admin.segment.DEFAULT_SORT`.
...
def decode_segment(raw: str) -> Result[SegmentModel]:
    ...
    nobody branches on, exactly as :func:`~hbd.db.admin.page.decode_cursor` collapses its six.
...
class SegmentFieldView(ApiModel):
    ...
    carried verbatim from :attr:`~hbd.db.admin.segment.FieldSpec.doc` rather than paraphrased
    ...
    #: :data:`~hbd.db.admin.segment.SORT_KEYS` rather than from ``FieldSpec.sortable``, which
...
class SegmentLimitsView(ApiModel):
    """:data:`~hbd.db.admin.segment.SEGMENT_LIMITS`, so the editor stops before the server does."""
...
class SegmentPreviewView(ApiModel):
    ...
    **The three refusal counts overlap and the client must not add them up.** ``blocked`` is
    ...
    #: Exact, never bounded — see :func:`~hbd.db.admin.users.count_segment_exactly` for why a
...
def to_segment_fields_view(*, capabilities: frozenset[str]) -> SegmentFieldsView:
    ...
    :func:`~hbd.db.admin.segment.segment_capabilities` here, so the router reads the
    deployment once and the same set decides both what this document says is available and
    what :func:`~hbd.db.admin.segment.compile_segment` will accept a moment later.
    """
