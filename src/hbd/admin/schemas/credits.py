...
``POST /reveal`` to gate and nothing for :mod:`hbd.admin.serializers.redaction` to hide; the
one identifier that appears, the Telegram id, is the value every ``/users/**`` route keys on
and that :class:`~hbd.admin.schemas.users.UserView` already ships in the clear beside its own
...
class CreditAccountView(ApiModel):
    ...
    ``creditsProjected`` on :class:`~hbd.admin.schemas.users.UserDetailView`. Both are on the
