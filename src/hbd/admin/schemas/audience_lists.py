...
:mod:`hbd.admin.schemas.overview`'s docstring states as a standing invariant that no
...
takes no window at all (:func:`hbd.db.admin.audience_lists.recent_subscribers` argues why).
...
from hbd.admin.schemas.dashboard import WindowView, to_window_view
...
class TopGeneratorView(ApiModel):
    ...
    **The pair is not a conversion rate**, and no consumer may form one from it. Both are
    ...
    count beside it, and even ``0``. :func:`hbd.db.admin.audience_lists.top_generators` says
...
class RecentSubscribersView(ApiModel):
    ...
    #: :func:`~hbd.db.admin.metrics.read_capabilities`, never declared.
...
def _to_recent_subscriber(item: RecentSubscriber, *, now: datetime) -> RecentSubscriberView:
    ...
    :func:`~hbd.db.admin.plan_purchases.plan_liability`, so a plan ending exactly now is
