...
A sibling of :mod:`hbd.admin.security.ratelimit`, deliberately: same fixed-window key shape,
...
:meth:`~hbd.admin.security.ratelimit.WindowCounterStore.increment_by` exists at all:
...
decision is a value, and ``hbd.admin.deps.enforce_reveal_budget`` maps it to the envelope.
"""
...
#: :mod:`hbd.admin.settings` imports this package, so importing it back is a cycle. The
...
_KEY_PREFIX: Final[str] = "hbd:admin:budget"
...
class RevealBudgetOutcome(StrEnum):
    ...
    :class:`~hbd.admin.security.ratelimit.RateLimitOutcome` splits them: one is an operator
...
class RevealBudgetLimits:
    ...
    imported *by* :mod:`hbd.admin.settings` and must not import it back — and because a
