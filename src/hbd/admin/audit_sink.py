...
``hbd.db.admin.audit.append`` is the only writer of ``admin_audit_log`` and it stays that
...
itself must leave neither, and ``hbd.admin.deps.get_db_session`` gives that for free by
