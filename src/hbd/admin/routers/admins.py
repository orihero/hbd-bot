...
§12.2 gives ADMIN_MANAGE to OWNER alone, so the other three roles are a 403 with an audit
row — and that refusal is written by the guard, not here. ``check_access`` would be the
wrong question for the same reason it is on the audit and retention lists: with no subject
there is no scope to compare a step-up against, so ``check_access(subject_id=None)`` would
log an ERROR on every request to a route that asks for no step-up at all. The step-up this
permission carries belongs to the Phase 2 writes, on the account they are about to change.
