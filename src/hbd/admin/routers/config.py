...
``hbd.config.Settings`` through an override layer, behind ``CONFIG_WRITE``, a step-up with
no grace (§12.1 T2) and ``admin_config_enabled``. The fields on this response are
``HBD_ADMIN_*`` variables, which are not on the bot's ``Settings`` at all — so they have no
...
:mod:`hbd.admin.schemas.config_view` rather than of this file: that module names every field
...
from hbd.admin.deps import API_PREFIX, Settings, require_permission
from hbd.admin.schemas.config_view import ConfigView, to_config_view
