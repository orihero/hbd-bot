...
guard resolves to :func:`~hbd.admin.security.permissions.check_role`, which holds no subject
...
and then enqueues an ARQ job through :mod:`hbd.admin.queue` — a Redis write and nothing more.
...
:class:`~hbd.admin.schemas.broadcasts.BroadcastRecipientView` publishes the mask alone —
...
from hbd.admin.schemas.segment import encode_segment, to_segment
...
from hbd.db.admin.broadcasts import (
    BroadcastFilters,
    RecipientFilters,
    count_broadcasts,
    count_recipients,
...
from hbd.db.admin.users import UserFilters, count_segment_exactly
from hbd.db.admin.views import BroadcastDetail
...
from hbd.db.broadcasts import cancel as cancel_broadcast
from hbd.db.broadcasts import mark_scheduled as schedule_broadcast
from hbd.db.broadcasts import pause as pause_broadcast
from hbd.db.broadcasts import resume as resume_broadcast
from hbd.db.enums import AuditAction, AuditReasonCode
...
#: :class:`~hbd.admin.schemas.broadcasts.BroadcastSendRequest` carries the instant or omits it.
...
#: :class:`~hbd.admin.schemas.broadcasts.BroadcastCreateRequest` asks for no reason code — but
#: :class:`~hbd.db.admin.audit.AuditEntry` requires one, deliberately, because a row that could
...
def _wrong_state(state: BroadcastState) -> ProblemError:
    ...
    ``UPDATE``s in :mod:`hbd.db.broadcasts` are the authority on that, and this reports the
...
def build_query(
    ...
    :data:`~hbd.db.admin.broadcasts._SEARCHABLE_COLUMNS`).
    """
...
def build_broadcast_actions_router() -> APIRouter:
    ...
    async def create_campaign(
        ...
        **The count is exact and never bounded.** It is the number a human will authorise a
        ...
        :func:`~hbd.db.admin.users.count_segment_exactly` makes that argument at length. It is
    ...
    async def revise_campaign(
        ...
        409 once the campaign has left the states :func:`~hbd.db.broadcasts.revise_broadcast`
...
# ---------------------------------------------------------------------------
#: A conditional state move in :mod:`hbd.db.broadcasts`. All three take the campaign and the
...
async def _reload(db: Db, broadcast_id: UUID) -> BroadcastDetail:
    ...
    here deletes it. Raised as a :class:`~hbd.errors.StorageError` so it lands in the shared
