...
:func:`~hbd.admin.routers.segments.compiled_segment` — the same pipeline
...
from hbd.admin.errors import AdminErrorCode, AdminProblem, ProblemError, problem, unwrap
from hbd.admin.routers.segments import SegmentToken, compiled_segment
from hbd.admin.schemas.actions import UserBlockRequest, UserBlockResultView
from hbd.admin.schemas.orders import OrdersPage, to_order_view
from hbd.admin.schemas.page import Paging, page_meta
from hbd.admin.schemas.segment import MAX_SEGMENT_KEY_CHARS
...
from hbd.db.admin.audit import AuditEntry
...
from hbd.db.admin.users import (
    UserFilters,
    count_users,
    get_user_detail,
    list_users,
...
from hbd.db.admin.views import UserListItem
from hbd.db.base import utc_now
from hbd.db.credits import set_blocked
...
#: ``hbd.admin.deps`` and drags ``AdminContainer``, the session factory and the permission
...
#: gets to sniff at.
#:
#: Built from :data:`hbd.user_profiles.AVATAR_MIME` rather than from a literal of our own,
...
#: is "no avatars anywhere" with a fully green suite. Importing from ``hbd.user_profiles`` is
#: legal: ``tests/test_admin/test_asset_stream.py:712`` caps only what this package imports
#: from ``hbd.storage``, and ``avatar_key`` already arrives the same way.
...
def _window(since: datetime | None, until: datetime | None) -> TimeWindow | None:
    ...
    :func:`~hbd.admin.window.resolve_window` makes both calls for all five routers; the
...
def build_filters(
    ...
    ``q`` carries ``max_length`` for the reason :func:`hbd.db.admin.sql.search_clause` gives:
    ...
    **What ``q`` searches is a privacy decision, and it is made in
    :class:`~hbd.db.admin.users.UserFilters`, not here.** It matches the Telegram id and
    ...
    ``credit_accounts`` row at all. :class:`~hbd.db.admin.users.UserFilters` argues why the
...
class UserQuery:
    ...
    and the default one take **different cursors** — :class:`~hbd.db.admin.page.Cursor` on
    :attr:`request` for ``(created_at, id)``, :class:`~hbd.db.admin.page.SortedCursor` on
    :attr:`cursor` for everything else — and :func:`~hbd.db.admin.users.list_users` raises
...
def build_query(
    ...
    **The segment is a document and the sort is a key**, and both name things the registry
    ...
    :data:`~hbd.db.admin.segment.SORT_KEYS` member — a **name**, resolved to an expression
    object by :func:`~hbd.db.admin.segment.sort_expression`, so no caller string ever reaches
    ...
    **``?sort=`` overrides the document's own ordering rather than sitting beside it** — see
    :func:`~hbd.admin.routers.segments.compiled_segment`, which is the single pipeline both
    routers run. One ordering, one source; :class:`~hbd.db.admin.users.UserFilters` refuses
    ...
    **The default sort keeps the cheap cursor, and that is not an optimisation.**
    ...
    :class:`~hbd.db.admin.page.Cursor`, and every bookmarked "next page" link an operator
    ...
    **``limit`` and ``cursor`` are declared here rather than taken from
    :data:`~hbd.admin.schemas.page.Paging`**, and the duplication is forced: that dependency
    ...
    constants, and :func:`~hbd.db.admin.page.page_request` still validates the limit a second
...
def _refuse_a_total_beside_an_aggregate_sort(filters: UserFilters, *, with_total: bool) -> None:
    ...
    :func:`~hbd.db.admin.users.count_segment_exactly` behind ``/segments/preview``.

    ``SORT_KEYS`` is indexed rather than ``.get``-ed: a sort that reached
    :class:`UserQuery` was resolved against that same mapping by
    :func:`~hbd.db.admin.users.page_sort`, so a miss here would be a bug in this module and
...
def _entitlement_policy(settings: AdminSettings) -> EntitlementPolicy:
    ...
    :func:`hbd.entitlements.resolve_entitlement_policy` now reads
    ...
    :func:`hbd.db.credit_sql.read_balance` adds ``policy.allowance_credits`` whenever an
    ...
    ``admin_settlement_grace_s`` in :mod:`hbd.admin.settings` — and neither may be defaulted
    here again.
    ...
    :mod:`hbd.config` — which this process deliberately cannot read (``settings.py``'s module
...
def build_users_router() -> APIRouter:
    ...
    async def list_user_records(
        ...
        :func:`~hbd.admin.schemas.users.to_user_view` did not already publish. Sorting on a
    ...
    async def get_user_record(db: Db, settings: Settings, telegram_user_id: int) -> UserDetailView:
        ...
        mirrored into :class:`~hbd.admin.settings.AdminSettings`. :func:`_entitlement_policy`
...
def build_user_block_router() -> APIRouter:
    ...
    :func:`~hbd.admin.security.permissions.check_role`, which holds no subject and therefore
