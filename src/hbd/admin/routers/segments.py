...
:data:`~hbd.db.admin.segment.FIELDS` is unaddressable under any spelling, and the four
free-text columns a campaign must never target are named in
:data:`~hbd.db.admin.segment.SEGMENT_REFUSALS` so that adding one back is a diff a reviewer
...
:func:`~hbd.db.admin.segment.compile_segment`'s refusal, and none of the four ever echoes the
...
from hbd.admin.errors import unwrap
...
from hbd.db.admin.users import UserFilters, segment_breakdown
...
#: inside :func:`~hbd.admin.schemas.segment.decode_segment` — the duplication ``q`` and
...
def compiled_segment(
    ...
    come from us", :func:`~hbd.admin.schemas.segment.to_segment` owns nothing at all (it is
    total), and the compiler owns every question about meaning. :func:`unwrap` renders both
    kinds as a 422, since :class:`~hbd.db.admin.segment.SegmentError` is a
    :class:`~hbd.errors.ValidationError`.
    ...
    **``sort`` REPLACES the document's own ordering rather than sitting beside it**, which is
    ...
    always carries exactly one sort, :class:`~hbd.db.admin.users.UserFilters` refuses a sort
    ...
    *before* compilation, where the registry still gets to refuse an unsortable key.
    ...
    thing :attr:`~hbd.db.admin.users.UserFilters.segment` means by ``None`` and is why an
...
def build_segment_fields_router() -> APIRouter:
    ...
    async def list_segment_fields() -> SegmentFieldsView:
        ...
        reads the ORM metadata (:func:`~hbd.db.admin.segment.segment_capabilities`), so this
...
def build_segments_router() -> APIRouter:
    ...
    async def preview_segment(db: Db, segment: SegmentToken = None) -> SegmentPreviewView:
        ...
        rather than an approximation (:func:`~hbd.db.admin.users.count_segment_exactly` makes
