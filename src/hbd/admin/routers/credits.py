...
:mod:`hbd.admin.security.permissions`.
...
same mechanical reason each time: :func:`~hbd.admin.security.permissions.check_role` is what a
...
from hbd.admin.routers.users import USER_PATH, USER_SUBJECT_TYPE, no_such_user
from hbd.admin.schemas.credits import (
    CreditGrantRequest,
...
from hbd.db.admin.credits import count_credit_entries, get_credit_account, list_credit_entries
from hbd.db.admin.users import UserFilters, count_users
...
def build_credits_router() -> APIRouter:
    ...
    async def list_user_credits(
        ...
        **It asks the credit subsystem as well as ``users``, and the order matters.** The
        ...
        must not render alike; :class:`~hbd.admin.schemas.credits.CreditLedgerPage` says which
