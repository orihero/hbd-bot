...
**This does not reuse ``hbd.errors.ErrorCode``, and that is a decision rather than an
...
Extending it would be worse than useless: ``HbdError`` carries a ``user_message_key`` and
...
:class:`ProblemError` carrying either an ``HbdError`` or an :class:`AdminProblem`, and
...
Every message is passed through ``hbd.logging.redact`` and capped before it reaches the
body. Two of the messages this envelope will eventually carry are built by interpolating an
operator-submitted value (``hbd.config``'s ``_describe_failure`` and
...
from hbd.contracts import Result, is_ok
from hbd.errors import ErrorCode, HbdError
from hbd.logging import current_correlation_id, get_logger, redact
...
class AdminErrorCode(StrEnum):
    """The HTTP surface's own closed taxonomy. Disjoint from ``hbd.errors.ErrorCode``."""
...
class ProblemError(Exception):
    ...
    Not an ``HbdError`` subclass: an ``HbdError`` promises a ``user_message_key`` that a
    customer-facing catalogue can render, and nothing in this package has one.
    """

    def __init__(self, failure: AdminProblem | HbdError) -> None:
        super().__init__(_describe(failure))
        self._failure = failure

    @property
    def failure(self) -> AdminProblem | HbdError:
        return self._failure


def _describe(failure: AdminProblem | HbdError) -> str:
    if isinstance(failure, HbdError):
...
def status_for(failure: AdminProblem | HbdError) -> int:
...
def unwrap[T](result: Result[T]) -> T:
    """The value, or a :class:`ProblemError` carrying the ``Err``'s ``HbdError``.
...
def _code_of(failure: AdminProblem | HbdError) -> str:
    return str(failure.error_code) if isinstance(failure, HbdError) else str(failure.code)


def _details_of(failure: AdminProblem | HbdError) -> Mapping[str, Any] | None:
    if isinstance(failure, HbdError):
        return None
    return failure.details


def render_envelope(failure: AdminProblem | HbdError, *, status: int | None = None) -> JSONResponse:
...
async def handle_unexpected(request: Request, exc: Exception) -> JSONResponse:
    ...
    **Public, and deliberately not registered by** :func:`install_error_handlers`. Starlette
    ...
    :mod:`hbd.admin.middleware.unhandled`). ``create_app`` installs this as that middleware's
