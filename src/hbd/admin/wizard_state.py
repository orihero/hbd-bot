...
``hbd.bot.app.build_storage`` and lives next to ``build_bot``, which takes
...
(:data:`hbd.bot.draft.DRAFT_KEY`), so the value at the data key is ``{"draft": {...}}``.
That one level is unwrapped here; the inner dict goes to
:func:`~hbd.admin.schemas.users.to_wizard_state_view`, which reads it through an allowlist
...
#: ``hbd.bot.app.build_storage`` hands it none. Constructed with no arguments on purpose:
