"""``python -m bayram.tools.identity show|set`` — the bot's public face, from the token alone.
...
def _parse(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m bayram.tools.identity",
...
async def _run(request: Request) -> str:
    ...
    if not settings.telegram_bot_token:
        raise RefusedError("BAYRAM_TELEGRAM_BOT_TOKEN is empty; this tool is the token or nothing.")
