"""``python -m bayram.tools.credits grant|block`` — the only operator write surface there is.
...
:meth:`bayram.entitlements.EntitlementStore.grant`, ``block`` through ``set_blocked``. Both
...
from bayram.db import ACTOR_LENGTH, SqlCreditLedger, create_engine, create_session_factory
...
class RefusedError(Exception):
    ...
    the right shape, and the same one :mod:`bayram.admin.bootstrap` uses. There is no caller to
...
def _parse(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m bayram.tools.credits",
