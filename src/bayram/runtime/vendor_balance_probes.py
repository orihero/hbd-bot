...
module is the half that does the asking. It imports ``httpx``, ``bayram.contracts``,
``bayram.errors`` and the speech transport, and **deliberately not ``bayram.db``**: every branch
...
:class:`bayram.providers.tts.elevenlabs_api.SubscriptionPayload` already parses the very
...
:func:`bayram.providers.tts.transport.send_request`, which already classifies a status into the
closed ``bayram.errors`` taxonomy — 401/403 to ``CONFIG_INVALID``, 402 to quota-exhausted, 5xx
...
from bayram.contracts import BalanceUnit, Err, Vendor
from bayram.errors import BayramError
from bayram.providers.tts.elevenlabs_api import API_KEY_HEADER, SUBSCRIPTION_PATH
from bayram.providers.tts.transport import http_status_of, parse_json_body, send_request
...
#: ``BAYRAM_LLM_BASE_URL`` ships versioned (``https://openrouter.ai/api/v1``) and OpenRouter's
...
class ElevenLabsSubscriptionPayload(BaseModel):
    ...
    Not :class:`bayram.providers.tts.elevenlabs_api.SubscriptionPayload`, and the difference is
...
def is_openrouter(base_url: str) -> bool:
    ...
    A bare host with no scheme is accepted too, because ``BAYRAM_LLM_BASE_URL`` is a string
    an operator types.
    """
