...
from bayram.config import Settings, env_file
from bayram.i18n import get_translator
...
def verify_host(settings: Settings) -> None:
    ...
    # ``environment`` and ``env_file`` are here because ``BAYRAM_ENV_FILE`` makes it possible to
