...
**This module is a leaf and must stay one.** It imports ``bayram.contracts`` and ``bayram.errors``
and nothing else, and it may never import ``bayram.db``. That is not tidiness: putting these
types in ``bayram.contracts`` was tried and reproduced a real circular import, because a
protocol that named ``bayram.db.enums.CreditReason`` made ``contracts`` import the ``bayram.db``
package, whose ``__init__`` imports ``bayram.db.attempts``, which imports ``bayram.contracts``.
...
* :class:`EntitlementStore` speaks :class:`SettlementOutcome`, not ``CreditEntryKind`` /
  ...
  ``bayram.db.credits``; the callers (the worker gate, the settlement hook, the CLI) never
  name a database enum, so the cycle cannot come back through a call site either.
* ``bayram.db`` imports *this* module, never the reverse. There is no back-edge to find.

``bayram.contracts`` is also already 857 lines, over this repo's own 800-line cap, which is
...
__all__ = [
    ...
    # They are DEFINED in bayram.errors because that module is the single error hierarchy and
...
#: The queue ladder as ``bayram.config`` ships it: ``queue_job_timeout_s`` (config.py:314),
#: ``queue_max_tries`` (config.py:320) and ``provider_backoff_base_s`` (config.py:214).
#: Restated as literals because this module is a LEAF and may not import ``bayram.config`` —
...
#: ``bayram.db.credits.settle_stale_debits`` is what closes such a debit properly.
#:
...
class CreditBalance(BaseModel):
    ...
    only way it can agree with what ``bayram.db.credits.charge`` will do a second later. Without
...
class EntitlementPolicy:
    ...
    #: when the balance is short, ``bayram.db.credits.charge`` covers the shortfall with a
...
def resolve_entitlement_policy(settings: object | None = None) -> EntitlementPolicy:
    ...
    :func:`bayram.db.retention.resolve_retention_policy` does it: this module is a leaf and
    importing ``bayram.config`` would give it an edge it has spent its whole docstring
    ...
    ``bayram.db.credits._mint_due_allowance`` return ``False`` immediately without touching the
    ...
    ``BAYRAM_SETTLEMENT_GRACE_S`` wins outright, and otherwise it is DERIVED from this
    deployment's queue ladder rather than left at the shipped default. That derivation is
    the point — an operator who raises ``BAYRAM_QUEUE_JOB_TIMEOUT_S`` to an hour has quietly
...
def _non_negative_int(settings: object, name: str) -> int | None:
    ...
    and ``BAYRAM_FREE_ALLOWANCE_CREDITS=false`` would otherwise read as a deliberate 0 rather
...
class EntitlementStore(Protocol):
    ...
    persistence implementation is ``bayram.db.credits.SqlCreditLedger``.
    ...
    async def forget(self, telegram_user_id: int) -> Result[None]:
        ...
        :func:`bayram.db.credit_erasure.forget_account` for what is deleted, what is kept
