...
This module is the canonical home. ``bayram.bot.payment`` re-exports it so the bot package
...
from bayram.entitlements import ChargeOutcome, EntitlementStore
...
class NoopPaymentProvider:
    """Always authorises. Structurally satisfies ``bayram.contracts.PaymentProvider``."""
    ...
    async def authorize(
        ...
        # has it (``bayram.contracts.PaymentProvider.authorize``), and in the log line because
...
class CreditGatedPaymentProvider:
    ...
    Structurally a ``bayram.contracts.PaymentProvider``, so it drops into the seam wherever the
    ...
    **It refuses with an ``Err``, never with ``is_authorized=False``.** Both gates already
    ...
    ``BayramError.user_message_key`` and interpolates the scalars in ``context`` — so an
    ...
    **It is the worker's provider, not the bot's.** ``AppContainer.pipeline`` wraps the
    ...
    ``bayram.db.purchases.SqlPurchaseLedger`` with ``actor=CHECKOUT_ACTOR``, wired into
    ``BotDeps.purchases`` by ``bayram.main``. Stated as "the bot must not write", the rule is
    ...
    async def _take_one_credit(
        ...
        *,
        ...
        if isinstance(charged, Err):
            ...
            # both are returned as-is: ``BayramError.is_retryable`` — False for every
