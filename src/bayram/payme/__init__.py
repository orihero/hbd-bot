...
**This package may import ``bayram.contracts``, ``bayram.checkout``, ``bayram.errors`` and
``bayram.logging``. Its wire, domain and port modules may NEVER import ``bayram.db``.** The
...
direction, exactly as ``bayram.db.purchases.SqlPurchaseLedger`` implements
``bayram.checkout.PurchaseFulfiller`` and ``bayram.db.credits.SqlCreditLedger`` implements
...
rooms:** :mod:`bayram.payme.container` (the composition root — a composition root is defined by
importing everything it wires), :mod:`bayram.payme.app` (which reaches for ``bayram.db.engine.ping``
and nothing else, for its readiness probe) and :mod:`bayram.payme.cli` (the operator tool, whose
cross-table diagnostics have no port and must not be given one on a process facing the public
internet). Every OTHER module here names ``bayram.db`` nowhere in its own source — including
:mod:`bayram.payme.harness`, which reaches persistence only through the container door — and
...
The second bar is narrower and easier to break by accident: **``bayram.payme`` must not import
``bayram.admin``.** They are different processes with different blast radii. The admin app holds
...
graph is what keeps the separation from being merely an intention — ``bayram.admin.app`` derives
...
* :mod:`bayram.payme.protocol` — the wire vocabulary. Method names, error codes, the two state
  enums, the millisecond clock conversion and the JSON-RPC envelope renderers. No I/O, no
  clock read, no credential.
* :mod:`bayram.payme.errors` — the refusals, as ``BayramError`` subclasses carrying an ``rpc_code``.
  They are ``BayramError`` subclasses SPECIFICALLY so ``bayram.db.guard.run_guarded`` already
  catches them and a refusal travels back to the dispatcher as an ``Err`` rather than as an
  exception crossing a seam.
* :mod:`bayram.payme.auth` — one constant-time comparison and one log-safe accessor.
* :mod:`bayram.payme.link` — the checkout URL, which is CONSTRUCTED and never requested.
* :mod:`bayram.payme.rules` — the pure predicates the persistence-side transaction body calls,
...
"what is in ``bayram.payme``?" is answerable without an ``ls``:

* :mod:`bayram.payme.ports` — the frozen views and the ``PaymeLedger`` protocol persistence
  implements from the other side. Still no I/O; it is the seam itself.
* :mod:`bayram.payme.settings` — ``PaymeSettings`` and its own ``BAYRAM_PAYME_ENV_FILE``. The one
  module that names the merchant key, and it holds it as a ``SecretStr``.
* :mod:`bayram.payme.service` — the dispatcher: one ``Result`` mapped onto one JSON-RPC envelope,
  one journal row and one log line per inbound call.
* :mod:`bayram.payme.app` — the ASGI application and the rule that every reply on ``/payme`` is
  HTTP 200, including a refusal.
* :mod:`bayram.payme.container` — the composition root. See the three-doors paragraph above.
* :mod:`bayram.payme.provider` — the BOT's half: a ``CheckoutProvider`` whose ``charge`` opens an
  intent and builds a URL, and opens no socket at all. It runs in the bot's process, not this
  one, which is why it may not see ``bayram.db`` either.
* :mod:`bayram.payme.pause` — the operator kill switch for NEW checkouts, read by the provider
  and never consulted on an inbound settlement.
* :mod:`bayram.payme.cli` — ``python -m bayram.payme.cli``: journal, statement, settle, notify,
  pause, resume, status, invariant, reconcile. The recovery surface that ships on day one.
* :mod:`bayram.payme.harness` — ``python -m bayram.payme.harness``: Payme's published sandbox
  scripts as a data table, replayed against a RUNNING gateway on certification day.

**Nothing in this package is reachable in a default deployment.** ``BAYRAM_CHECKOUT_PROVIDER``
ships ``stub`` and ``BAYRAM_PAYME_ENABLED`` ships false, so production behaviour is byte-identical
