...
exactly one place, ``bayram.payme.provider.PaymeCheckoutProvider.charge``, which is the bot-side
...
a Redis failure into a :class:`bayram.errors.StorageError` rather than swallowing it, because the
...
``docs/deployment/08-payme.md``: ``python -m bayram.payme.cli status`` reports the switch, and the
...
``bayram.ratelimit.WindowCounterStore``): it lets the two functions below be tested against a fake
that can be made to fail on demand, which is the only way the two paragraphs above can be
asserted rather than merely believed. It also keeps ``bayram.payme`` free of a runtime dependency
...
#: The one key. Namespaced ``bayram:`` like everything else this deployment writes into a shared
...
PAYME_PAUSE_KEY: Final[str] = "bayram:payme:paused"
...
#: running ``redis-cli GET bayram:payme:paused`` during an incident should see something that
...
#: difference between ``redis-cli SET bayram:payme:paused 0`` quietly pausing the rail — the exact
...
class PauseSwitch(Protocol):
    ...
    Narrow on purpose, following :class:`bayram.ratelimit.WindowCounterStore`: a parameter typed
    ...
    ``bayram.payme.container`` passes its ``ArqRedis`` straight in.
    """
...
async def set_paused(switch: PauseSwitch, *, paused: bool) -> None:
    """Flip the switch. **Raises :class:`bayram.errors.StorageError` when the write does not land.**
