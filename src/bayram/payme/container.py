...
**This is one of the three modules in ``bayram.payme`` allowed to import ``bayram.db``, and the
...
two doors are narrower: :mod:`bayram.payme.app` reaches for ``bayram.db.engine.ping`` and nothing
else, and :mod:`bayram.payme.cli` runs the cross-table diagnostics that have no port and must not
be given one on a process facing the public internet. Every OTHER module here names ``bayram.db``
...
**It is emphatically NOT ``bayram.runtime.container.build_container``.** That function builds a
...
``bayram.config.Settings``, which is the model that holds all four outbound credentials. A payment
...
prevent. The absence of an HTTP client here is the same control ``bayram.admin.container`` makes:
...
inherit it.** ``bayram/db/engine.py:40-41`` sizes the bot and the worker at ``10 + 20`` each;
``bayram/admin/container.py:53-54`` sizes the admin API at ``5 + 5``. That is ``30 + 30 + 10 =
...
from bayram.db.payme_sql import insert_rpc_log
from bayram.logging import get_logger
from bayram.payme.ports import PaymeLedger
from bayram.payme.service import NotifyEnqueue, PaymeService, RpcJournal
from bayram.payme.settings import PaymeSettings
...
#:
#: **Restated here rather than imported from ``bayram.runtime.payme_jobs``, on purpose.** Importing
#: it would pull ``bayram.runtime`` into this process, and that package's ``__init__`` chain
...
#: this gateway's post-commit enqueue and ``bayram.runtime.payme_jobs.run_payme_sweep``'s backstop
...
#: is materialised directly — exactly as ``bayram.runtime.container`` and ``bayram.admin.container``
#: both do it.
...
class SqlRpcJournal:
    ...
    Satisfies :class:`bayram.payme.service.RpcJournal` structurally, matching every other adapter
...
class PaymeContainer:
    ...
    ``bayram.checkout.PaymentIntentOpener`` so the bot cannot settle a payment — and here it means
    ...
    async def aclose(self) -> None:
        ...
        try:
            ...
            # suppression ``bayram.admin.container`` carries, for the same stub.
...
async def build_payme_container(
    ...
    # (``bayram.checkout.PaymentIntentOpener``), performed in the bot's process under the bot's
