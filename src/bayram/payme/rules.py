...
from bayram.payme.protocol import MISSING_TIME, PaymeErrorCode, PaymeState, WireState, to_ms
...
#: Twelve hours in milliseconds: the DEFAULT of ``BAYRAM_PAYME_TRANSACTION_TIMEOUT_MS``, and
...
def is_expired(*, payme_time: datetime, now: datetime, timeout_ms: int) -> bool:
    ...
    **The boundary is exclusive: at exactly ``timeout_ms`` the window is still OPEN.** The two
    ...
    Both instants go through :func:`bayram.payme.protocol.to_ms` rather than being subtracted as
