...
allowed to reach.** ``bayram.config.Settings`` holds the five outbound vendor credentials;
``bayram.admin.settings.AdminSettings`` deliberately holds none of them; and this model holds
...
**``BAYRAM_PAYME_ENV_FILE`` is a SECOND variable and not a tier name shared with
``BAYRAM_ENV_FILE``.** The rule is stated once, at :data:`bayram.admin.settings.ADMIN_ENV_FILE_VAR`,
...
:mod:`bayram.payme.app` follows the same selection, so the file that was LOADED is the file that
gets scanned.

**There are no aliases on any field.** The environment variable name is mechanically
``BAYRAM_`` + the field name upper-cased, with no exceptions, because
...
* :attr:`PaymeSettings.payme_merchant_key` is ``SecretStr`` rather than ``str``, following
  ...
  shape ``bayram.logging.redact`` cannot mask: that function needs the field NAME beside the
...
* **No length assertion on the key.** The documentation says 36 characters; the length of the
  ...
  against itself (:func:`bayram.payme.auth.verify_basic`), so its length is not a correctness
  property of anything here. :mod:`bayram.payme.app` logs the length — never the value — at INFO
...
* :attr:`PaymeSettings.payme_return_url` refuses any value containing ``;``. That character is
  ...
  is at settings-build time, which is why :func:`bayram.payme.link.build_checkout_link` is total
...
#: Selects a different one — ``BAYRAM_PAYME_ENV_FILE=/etc/bayram/payme.env``, which is what
#: ``deploy/systemd/bayram-payme.service`` sets. Deliberately its OWN variable rather than a tier
#: name shared with ``BAYRAM_ENV_FILE``; see the module docstring and
#: :data:`bayram.admin.settings.ADMIN_ENV_FILE_VAR`, which states the rule first. Read from the
#: process environment only, for the reason on :data:`bayram.config.ENV_FILE_VAR`.
...
def _split_csv(value: Any) -> Any:
    ...
    Mirrors the helper in :mod:`bayram.config` and the one in :mod:`bayram.admin.settings` rather
...
class PaymeSettings(BaseSettings):
    ...
    ``bayram.config.build_settings`` offers the bot and ``build_admin_settings`` offers the panel.
    """
    ...
    #: :class:`bayram.payme.service.PaymeService`.
    ...
    #: unit file's ``ExecStart``, never by :func:`bayram.payme.app.create_app` — uvicorn is told
    ...
    #: "derive" (:func:`bayram.payme.link.resolve_base_url`). Read here by the operator CLI and
    ...
    #: ``bayram.admin.security.clientip`` learned this already, at the cost of a rate limiter
    ...
    def _a_live_rail_has_a_key(self) -> Self:
        ...
        depends on — :mod:`bayram.payme.app` logs the length at INFO instead, where a truncated
...
def _describe_failure(exc: PydanticValidationError) -> str:
    ...
    ``loc`` — so it is printed as the message alone rather than as ``BAYRAM_<ROOT>``, which names
...
def build_payme_settings(overrides: Mapping[str, object] | None = None) -> PaymeSettings:
    ...
    operator message names every offending ``BAYRAM_*`` variable. Never carries a VALUE into the
