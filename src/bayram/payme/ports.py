...
:mod:`bayram.payme.protocol` says what the wire looks like. This module says what the gateway is
...
exists — the implementation lives in ``bayram.db.payme`` and satisfies :class:`PaymeLedger`
structurally, in exactly the direction ``bayram.db.purchases.SqlPurchaseLedger`` satisfies
``bayram.checkout.PurchaseFulfiller``. That direction is the whole architecture: a rail that
...
**Why a SECOND port when ``bayram.checkout.PaymentIntentOpener`` already exists.** They are two
...
compensating write is needed — but ``bayram.db.guard.run_guarded`` turns each into an ``Err``
before it crosses this boundary. See :mod:`bayram.payme.errors` for the full argument.
...
:class:`bayram.checkout.PaymentIntent` is reused rather than re-declared for the intent, because
...
#:
#: It lives HERE, in ``bayram.payme``, rather than in either of the two modules that use it,
#: because those two may not import each other: the bot's provider is in this package and may
#: never import ``bayram.db``, while the ledger is in ``bayram.db`` and imports this package freely.
...
#: :data:`bayram.db.fulfilment.CHECKOUT_ACTOR` makes for the ledger's actor, and for the identical
...
#: What ``payment_intents.settle_note`` starts with when a HUMAN settled the intent instead of
#: an inbound ``PerformTransaction``. Written by ``bayram.db.payme.SqlPaymeLedger._force_settle``
#: as ``operator:<ref>``; read by everything that has to tell the two populations apart.
#:
#: It lives here for :data:`PAYME_PROVIDER_NAME`'s reason and for one sharper one. The
#: reconciliation identity is ``transactions_performed + operator_settlements ==
#: receipts_written``: a force-settled intent has no performed transaction, so the second term
#: is the ONLY thing that stops every use of the recovery button reading as a defect. That term
#: is selected by ``settle_note LIKE 'operator:%'`` and by nothing else, so the moment two
#: modules spell the prefix differently the invariant starts reporting incidents that are not
#: incidents — and a monitoring line nobody trusts is worse than no line at all. It was spelled
#: three times before this constant existed (the ledger privately, the CLI again "because the
#: ledger keeps it private", and the admin read layer would have been the third); making it
#: PUBLIC here answers that objection instead of adding a fourth copy, because ``bayram.db`` and
#: ``bayram.payme.cli`` both import this module already and neither imports the other.
...
class PaymeTransactionView:
    ...
    **The three clocks are separate columns and they are carried here as three fields**, not
    ...
    :func:`bayram.payme.rules.wire_time` is the one place that coercion is applied. A column
...
class SettlementCounts:
    ...
    (:func:`bayram.db.fulfilment.write_single_sale`), and their whole purpose is to notice that a
    ...
    * ``grants_written`` is the SINGLE-song subset of ``receipts_written`` by construction, not
      ...
      credit at all (``bayram.db.fulfilment.write_plan_sale`` — a plan mints its songs as they are
...
class PaymeLedger(Protocol):
    ...
    The superset of ``bayram.checkout.PaymentIntentOpener``, and the asymmetry is the security
    ...
    **``now`` is a parameter on every method that has an opinion about time.** The rail's
    ...
    async def cancel(
        ...
        it — see :class:`bayram.payme.errors.PaymeOrderDelivered` and ``PAYME_INTEGRATION §6``.
