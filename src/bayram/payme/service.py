...
Every business answer comes back from :class:`bayram.payme.ports.PaymeLedger` as a ``Result``;
...
burned.** A ledger ``Err`` carrying a :class:`bayram.payme.errors.PaymeFault` renders that fault's
...
``ValidationError``, an ``BayramError`` nobody anticipated — renders ``-32400``, because "we
...
the rail believes failed and retries. The sweep in ``bayram.runtime.payme_jobs`` re-enqueues
...
from bayram.payme.errors import PaymeAccountFault, PaymeFault
from bayram.payme.ports import PaymeLedger, PaymeStatementRow, PaymeTransactionView
...
from bayram.payme.rules import wire_state, wire_time
...
class RpcJournal(Protocol):
    ...
    A port rather than a direct call into ``bayram.db.payme_sql.insert_rpc_log`` for two reasons,
    ...
    The second is that :mod:`bayram.payme` may not import :mod:`bayram.db` (see the package
    docstring), so the implementation lives beside the engine that serves it, in
    :mod:`bayram.payme.container`.
    """
...
def _received_account_fields(params: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    «Настройка Аккаунт» form and ``BAYRAM_PAYME_ACCOUNT_FIELD`` agree.
    """
...
def _fault_message(error: BayramError) -> str:
...
class PaymeService:
    ...
    # -- refusals -----------------------------------------------------------
    def _fault(self, error: BayramError, *, request_id: int) -> dict[str, object]:
        ...
        **The envelope's SHAPE is decided by the code's range, not by the exception class**,
        ...
        its own subfield name. One does not: ``BAYRAM_PAYME_DUPLICATE_TRANSACTION_CODE`` is raised
...
#: Which codes :data:`bayram.payme.protocol.ACCOUNT_MESSAGES` actually carries a message for.
...
def _account_message(code: int) -> Mapping[str, str]:
    ...
    :data:`bayram.payme.protocol.ACCOUNT_MESSAGES` covers the five codes the ledger raises. The
    fallback exists because the code can also arrive through
    ``BAYRAM_PAYME_DUPLICATE_TRANSACTION_CODE``, which certification may point at a member of this
