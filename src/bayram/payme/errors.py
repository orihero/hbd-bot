"""The rail's refusals, as ``BayramError`` subclasses that each carry a JSON-RPC code.
...
wrapped in :func:`bayram.db.guard.run_guarded`, which catches ``BayramError`` and hands the caller
an ``Err``. The refusal crosses the boundary as a value; it just does not START as one.

**They are ``BayramError`` subclasses SPECIFICALLY so that ladder already catches them.**
``run_guarded`` catches ``NotFoundError``, then ``BayramError``, then ``IntegrityError``,
...
PAYME's screen — rendered from :data:`bayram.payme.protocol.ACCOUNT_MESSAGES` by the dispatcher,
...
class here inherits ``BayramError``'s generic ``error.generic`` key untouched rather than
declaring one. (The obvious alternative, ``default_user_message_key = None``, is not
available: ``BayramError`` declares that attribute as ``str`` and ``mypy --strict`` rejects the
...
from bayram.payme.protocol import DEFAULT_ACCOUNT_FIELD, PaymeErrorCode
...
class PaymeFault(BayramError):  # noqa: N818
    ...
    distinct from ``BayramError``, which at this seam means "something broke and Payme gets
    ...
    def with_context(self, **extra: Any) -> BayramError:
        ...
        ``BayramError.with_context`` rebuilds the error by calling ``type(self)(...)`` with the
...
class PaymeStateRefusal(PaymeFault):
    ...
    **The code is settable at construction for exactly one case**, and it is the one place
    ...
    it through ``BAYRAM_PAYME_DUPLICATE_TRANSACTION_CODE`` so a certification finding is an
...
class PaymeAccountFault(PaymeFault):
    ...
    equal ``BAYRAM_PAYME_ACCOUNT_FIELD``. Payme's interface uses it to highlight which field of
    ...
    :data:`bayram.payme.protocol.DEFAULT_ACCOUNT_FIELD` so a test or a CLI can raise one without
