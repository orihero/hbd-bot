...
(``src/bayram/admin/schemas/reveal.py:182``, ``:282``) and the step-up scope is spelled
...
may be no ``users`` row to point at — is answered by ``bayram.db.users_sql.ensure_user``, which
...
**There is no ``avatar_storage_key`` column.** The key is ``bayram.user_profiles.avatar_key``
...
from bayram.db.base import Base, TimestampMixin, UtcDateTime
...
#: :func:`bayram.user_profiles.normalise_phone` ever emits. Sixteen is therefore the exact
...
#: An IANA media type with parameters. Only :data:`bayram.user_profiles.AVATAR_MIME` is ever
...
class UserProfileRow(TimestampMixin, Base):
    ...
    data, which is what makes the whole-row DELETE in ``bayram.db.user_profiles`` a complete
    ...
    #: :func:`bayram.user_profiles.normalise_phone` produces. Deliberately **not** indexed and
    ...
    #: this value whole against :data:`bayram.user_profiles.AVATAR_MIME` and 415s otherwise.
    ...
    #: :func:`bayram.user_profiles.avatar_key` of ``user_id``, rebuilt wherever it is needed,
