...
``bayram.db.purchases._fulfil_single`` writes exactly one ``credit_ledger`` GRANT
...
#: ``max_length``, so a receipt that validated in ``bayram.checkout`` cannot fail its INSERT.
TOPUP_PROVIDER_LENGTH: Final[int] = 32
#: The rail's identifier for the charge. Mirrors ``Purchase.reference``'s ``max_length`` for
#: the same reason: the two widths are one decision, made in ``bayram.checkout``, stored here.
