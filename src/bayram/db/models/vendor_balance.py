...
``bayram.providers.tts.elevenlabs_api.SubscriptionPayload`` declares ``character_count: int =
...
for ``purge_runs.vendor_usage_deleted``, and the same one ``bayram.db.admin.vendor_usage``
...
reset hint) or a bounded error code from the ``bayram.errors`` taxonomy. No name, no note, no
...
left silent. **No retention cutoff either, and nothing is added to ``bayram.db.purge``:** the
...
from bayram.contracts import BalanceEstimateBasis, BalanceUnit, Vendor
...
#: Mirrors ``vendor_usage.ERROR_CODE_LENGTH``: a short symbolic name from the ``bayram.errors``
...
class VendorBalanceRow(Base, TimestampMixin):
    ...
    #: OpenRouter, since all four ``BAYRAM_LLM_*_USD_PER_MILLION_*`` rates default to 0.0, so
    ...
    #: From the closed ``bayram.errors`` taxonomy, never a body excerpt. A ``config_invalid``
