...
had paid 7 000 soʻm for. So nothing is granted up front: ``bayram.db.credits._mint_plan_song``
...
* **``telegram_user_id`` is nullable so that ``/forget`` has somewhere to go.** Every writer
  supplies it; a ``NULL`` means one thing only — ``bayram.db.credit_erasure.forget_account`` ran
...
#: ``max_length`` so a receipt that validated in ``bayram.checkout`` cannot then fail its INSERT.
PLAN_PROVIDER_LENGTH: Final[int] = 32
#: The rail's identifier for the charge. Mirrors ``Purchase.reference``'s ``max_length`` for
#: the same reason: the two widths are one decision, made in ``bayram.checkout`` and stored here.
...
class PlanPurchaseRow(Base, TimestampMixin):
    ...
    this row is genuinely UPDATEd — :func:`bayram.db.plan_sql.claim_plan_song` bumps
    ...
    #: back (``bayram.db.credits.charge`` argues why).
