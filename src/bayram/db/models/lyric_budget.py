...
that increments them (``bayram.db.lyric_budget.claim_write``). There is nothing to purge
because nothing accumulates.

**No foreign key to ``users``, and a natural primary key**, for the reason
``credit_accounts`` gives at ``src/bayram/db/models/credit_account.py:5``: ``users`` rows are
...
commands ``bayram.bot.gate.ERASURE_COMMANDS`` exempts from the inbound throttle, so that reset
would be available as fast as a script could type it. This is the same trade
``bayram.db.credit_erasure`` already makes and argues for the allowance's idempotency key —
...
from bayram.db.base import Base, TimestampMixin
...
class LyricBudgetRow(TimestampMixin, Base):
    ...
    #: :func:`bayram.lyric_budget.day_index_for`, which is the allowance's own window
