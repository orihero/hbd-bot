...
``bayram.db.retention`` delete the recipient's name, the customer's note and the rendered
...
class CreditLedgerRow(Base):
    ...
    #: (``bayram.db.credit_erasure.forget_account``). The row survives because the count is
