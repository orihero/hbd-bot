...
from bayram.db.base import Base, TimestampMixin, UtcDateTime, enum_type, utc_now
...
class UserRow(TimestampMixin, Base):
    ...
    **The DDL freeze was broken once, deliberately, and the reconciliation belongs here so
    ...
    home for a phone number and a face (:mod:`bayram.db.models.user_profile`) — and that half
    is untouched.
    ...
    **Rows are born at first contact, not at first order**, and there are three writers.
    ``bayram.db.users_sql.ensure_user`` is called from ``repository._create_order`` when an
    order is confirmed and from ``bayram.db.user_profiles.SqlUserProfiles.record_language``
    ...
    ``bayram.db.churn.mark_bot_blocked`` / ``mark_bot_unblocked``, which UPSERTs the row for the
    ...
    **The identity facts the bot now collects are NOT here.** The phone number, the
    ...
    row's ``id`` — see :mod:`bayram.db.models.user_profile` for the full argument. In one
    ...
    #: Written ONLY by ``bayram.db.churn.mark_bot_blocked`` / ``mark_bot_unblocked``, and it
