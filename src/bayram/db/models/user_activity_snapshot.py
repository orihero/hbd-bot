...
``bayram.db.purge``, a ``RetentionPolicy`` period and a ``/forget`` decision — and it would
...
in ``bayram.db.purge``, no ``PurgeReport`` field, no ``PurgeRunRow`` column, no entry in
``rows_past_expiry_statements`` and no ``RetentionPolicy`` field — and ``bayram.db.purge``'s
