...
This is the table the name subsystem is tuned from. ``BAYRAM_NAME_CANDIDATE_ORDER`` is
configuration precisely so a finding from this table can be applied without a code change;
:meth:`bayram.db.attempts.GenerationAttemptRepository.strategy_stats` is the query that
...
from bayram.contracts import MAX_CANDIDATE_CHARS, CostSource, Language, NameStrategy
