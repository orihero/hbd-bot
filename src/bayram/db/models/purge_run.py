...
* **A sweep that deleted rows but not the bytes behind them.** ``purge_expired`` returns
  ...
  ``bayram.db.purge``'s module docstring), so the number of keys handed over and the number
...
are still bounded, at 365 days, by ``bayram.db.purge._purge_purge_runs``.
"""
...
#: ``BayramError.error_code`` is a short symbolic name, never a message.
