...
wire boundary in ``bayram.payme.protocol``, not a column type.
...
class PaymeTransactionRow(Base, TimestampMixin):
    ...
    #: :class:`bayram.db.enums.PaymeState`. Moved only by conditional ``UPDATE``s that name the
    ...
    #:
    #: **Indexed by revision 0025.** It is the ``transactions_performed`` half of
    #: ``payme_sql.settlement_counts``, whose other half filters ``payment_intents.settled_at``
    #: — the same instant by construction, and now indexed on both sides for the same reason:
    #: the invariant is recomputed by the five-minute sweep AND by an admin screen that polls,
    #: and neither may be a full scan of this table. Highly selective too, since it is NULL for
    #: every transaction that was created and then cancelled. The name is spelled identically
    #: in the migration because ``test_the_migrated_indexes_match_the_model_metadata`` compares
    #: index NAMES.
    perform_time: Mapped[datetime | None] = mapped_column(UtcDateTime, nullable=True, index=True)
