...
:class:`~bayram.contracts.Language`, a storage key, a Telegram file id, a URL, or operator-
...
from bayram.db.base import Base, TimestampMixin, enum_type
...
#: ``MAX_MESSAGE_CHARS`` holds. Spelled here rather than imported: a model in ``bayram.db`` must
#: not depend on ``bayram.bot``, and the two are pinned to each other by the sender that would
