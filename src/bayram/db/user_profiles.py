...
The shape is :mod:`bayram.db.lyric_budget`'s and :mod:`bayram.db.credits`', copied deliberately
...
* the **free functions** below take an ``AsyncSession`` first and positionally, take ``now``
  ...
  :func:`bayram.db.users_sql.ensure_user`, :func:`upsert_language` and :func:`load_profile` as
...
* :class:`SqlUserProfiles` is the never-throw facade: every public method is one
  :func:`bayram.db.guard.run_guarded` delegation owning exactly one
...
holds :class:`bayram.user_profiles.UserProfileStore`, a protocol over ``Result``, and the admin
panel reaches the same table through :mod:`bayram.db.admin.users`' read-only projections.
"""
...
from bayram.contracts import Language, Result, Storage, is_err
...
from bayram.user_profiles import AVATAR_MIME, UserProfile, avatar_key

__all__ = [
    # The facade the bot holds through ``bayram.user_profiles.UserProfileStore``.
    "SqlUserProfiles",
...
class ProfileErasure:
    ...
    they are diagnostic, exactly as :class:`bayram.db.credit_erasure.CreditErasure`'s are.
    """
...
async def upsert_language(
    ...
    *,
    ...
    :func:`bayram.db.users_sql.ensure_user` states about the row above it: this is the FIRST
    ...
    **``language_chosen_at`` is write-once, via ``COALESCE`` in the ``SET`` clause.** The
    ...
    :func:`bayram.db.lyric_budget.claim_write` states the same fact and its ``CASE`` depends on
...
async def upsert_contact(
    ...
    **Nothing is re-validated here, deliberately.** The caller has already checked that
    ...
    :func:`bayram.user_profiles.normalise_phone`. A second definition of "a valid number" living
...
async def erase_profile(session: AsyncSession, *, telegram_user_id: int) -> ProfileErasure:
    ...
    **A DELETE, not a blanking.** Nulling columns leaves a row that half exists, and every
    ...
    erasure record, and :mod:`bayram.db.models.user_profile`'s docstring carries the full
...
class SqlUserProfiles:
    """:class:`bayram.user_profiles.UserProfileStore` over ``user_profiles`` and ``users``.

    Owns its transactions, owns its clock, owns the object store, and **never raises**: every
    public method is one :func:`bayram.db.guard.run_guarded` delegation, so a dropped connection
    ...
    **Why the ``Storage`` handle is on THIS class and not on ``BotDeps`` (D13).** The avatar
    ...
    :mod:`bayram.contracts`, never ``bayram.storage.LocalFileStorage``: the unit suite substitutes a
    ...
    **There is deliberately no retention policy argument here (PD-2.)** No clock, no
    ...
    for a policy to parameterise. :mod:`bayram.db.models.user_profile`'s docstring carries the
    ...
    async def _read_back(self, session: AsyncSession, telegram_user_id: int) -> UserProfile:
        ...
        identity-map copy. It raises :func:`bayram.db.guard.not_found` anyway instead of casting,
