...
Why this table is not bookkeeping: ``BAYRAM_NAME_CANDIDATE_ORDER`` is configuration rather
...
* ``name_candidate_text`` / ``stt_transcript`` **are** the recipient's name. They carry the
  90-day identity clock and :func:`bayram.db.purge.purge_expired` nulls them in place.
...
from bayram.db.models.generation_attempt import TRANSCRIPT_LENGTH, GenerationAttemptRow
...
class GenerationAttemptRepository:
    ...
    async def strategy_stats(
        ...
        **The window is what makes the answer current.** Without one, a strategy retired six
        ...
        answer — what should ``BAYRAM_NAME_CANDIDATE_ORDER`` be *now* — is answered with
        ...
        half-open :class:`~bayram.db.admin.sql.TimeWindow` instead — the one way a window is
