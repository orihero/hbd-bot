"""Persistence for Bayram: mapped rows, repositories, and the retention purge.
...
:class:`bayram.db.retention.RetentionPolicy`, and every write stamps its own ``expires_at``
...
from bayram.db.attempts import (
    GenerationAttempt,
    GenerationAttemptRepository,
    StrategyStat,
)
from bayram.db.base import Base, utc_now
...
from bayram.db.guard import NotFoundError
...
from bayram.db.purge import PurgeReport, purge_expired
...
__all__ = [
    ...
    # ``SqlCreditLedger`` returns the frozen ``bayram.entitlements`` view models instead, so a
    ...
    # ``bayram.checkout.PaymentIntentOpener``, which declares ``open_intent`` and nothing else, so
    ...
    # object typed as ``bayram.payme.ports.PaymeLedger``, which declares the settlement half.
    #
    ...
    # nothing else in the tree may reach past this module into ``bayram.db.payme_sql``.
    "SqlPaymeLedger",
    ...
    # import because ``bayram.runtime.providers`` is the only caller that builds one, and it
