...
division of labour; :mod:`bayram.db.models.bot_membership_event` argues it in full.
...
**Why an UPSERT of the ``users`` row first.** :func:`bayram.db.credits.set_blocked` states the
...
Shape follows :mod:`bayram.db.credits` and :mod:`bayram.db.lyric_budget`: the module-level
...
from bayram.contracts import BotBlockSource, BotMembershipEvent, Result
...
async def mark_bot_blocked(
    ...
    UPDATE matched, and :func:`bayram.db.credit_sql.insert_or_ignore` is used precisely because
    it reports which.
...
async def anonymise_bot_membership_events(session: AsyncSession, *, telegram_user_id: int) -> int:
    ...
    Mirrors :func:`bayram.db.plan_sql.anonymise_plans` exactly, including that it does not
    commit: the caller (:func:`bayram.db.credit_erasure.forget_account`) owns the transaction so
...
class SqlBotBlocks:
    """:class:`bayram.churn.BotBlockRecorder` over ``users`` and ``bot_membership_events``.
    ...
    ``bayram.db.admin`` and the only writers are the two above.
    """
