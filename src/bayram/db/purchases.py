...
stays literally true with one stated, bounded exception. :class:`bayram.checkout.PurchaseFulfiller`
...
* **A single song writes ONE RECEIPT AND ONE CREDIT.** ``topup_purchases`` records what was
  ...
  while two statements carry it instead of one. :mod:`bayram.db.models.topup_purchase` argues
...
* **A plan grants NOTHING and writes ONE ROW.** ``plan_purchases`` records what was bought,
  ...
  (:func:`bayram.db.credits._mint_plan_song`). An eager twelve-credit grant would need an expiry
  ...
  money or a top-up the customer paid cash for. :mod:`bayram.db.plan_sql` argues it at length.

Shape follows :class:`bayram.db.credits.SqlCreditLedger` exactly: built from an
``async_sessionmaker`` plus an injected clock, one ``async with self._sessions.begin()`` per
public method, every one of them wrapped in :func:`bayram.db.guard.run_guarded` so it returns
...
**What this class is, now that :mod:`bayram.db.fulfilment` exists: a transaction, a clock, a
...
from bayram.checkout import Plan, PlanState, Purchase
from bayram.contracts import Result
from bayram.db.base import utc_now
from bayram.db.credit_sql import read_balance
from bayram.db.fulfilment import CHECKOUT_ACTOR, write_plan_sale, write_single_sale
from bayram.db.guard import run_guarded
from bayram.db.models.plan_purchase import PlanPurchaseRow
from bayram.db.plan_sql import current_plan
from bayram.entitlements import DEFAULT_ENTITLEMENT_POLICY, CreditBalance, EntitlementPolicy
...
def _plan_state(row: PlanPurchaseRow) -> PlanState:
    ...
    ``PlanKind`` and :class:`bayram.checkout.Plan` are separate enums that mirror each other
...
class SqlPurchaseLedger:
    ...
    Structural implementation of :class:`bayram.checkout.PurchaseFulfiller`, not a subclass: the
    ...
    The ``policy`` is carried for one reason only: :func:`bayram.db.credit_sql.read_balance`
    ...
    async def _fulfil_single(
        ...
        :func:`bayram.db.fulfilment.write_single_sale`, which every rail shares. What is left
        ...
        **The balance read is INSIDE the same transaction and after the writes**, so the
        ...
        :class:`bayram.entitlements.EntitlementPolicy` and ``write_single_sale`` does not:
        :func:`bayram.db.credit_sql.read_balance` projects a due allowance and a live plan's
    ...
    async def _start_plan(
        ...
        The sale is :func:`bayram.db.fulfilment.write_plan_sale`, which every rail shares and
        ...
        into the frozen :class:`bayram.checkout.PlanState` a bot screen is drawn from, which is
