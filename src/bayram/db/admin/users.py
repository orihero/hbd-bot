...
* ``bayram.db.users_sql.ensure_user`` — called from ``repository._create_order`` when an order
...
* ``users.last_seen_at`` does now have a real writer, and this module still refuses to
  ...
  records. The list reports :attr:`~bayram.db.admin.views.UserListItem.last_order_at` instead,
...
The detail additionally calls :func:`bayram.db.credit_sql.read_balance`, which is what the
bot's own gate calls, with the deployment's own :class:`~bayram.entitlements.EntitlementPolicy`
...
:mod:`bayram.db.admin.segment` compiles a document into one boolean expression, this module
...
page a :class:`~bayram.db.admin.page.SortedCursor` walk (:func:`_sorted`), and "how many people
...
class UserFilters:
    ...
    **:attr:`search` is that refusal restated for substrings, and a substring is worse.** An
    ...
    is not a limitation of the helper: :func:`bayram.db.admin.sql.search_clause` would search
    ...
    **:attr:`segment` does not widen any of that, because the registry it compiles against is
    ...
    builder, so a field nobody wrote into :mod:`bayram.db.admin.segment` is unaddressable under
    ...
    **``users.last_seen_at`` is the one asymmetry, and it is deliberate.** The registry lets a
    ...
    #: :func:`~bayram.db.credit_sql.read_balance`'s projection: the projection is three statements
    ...
    #: A COMPILED segment — :func:`bayram.db.admin.segment.compile_segment`'s output, never a
    ...
    #: keyset this list has always used and the smaller :class:`~bayram.db.admin.page.Cursor`
    ...
    #:
    #: It is the PAGINATION layer's :class:`~bayram.db.admin.page.SortSpec`, not the segment
...
def page_sort(sort: SegmentSortSpec) -> tuple[SortSpec, SortValueKind]:
    ...
    :class:`bayram.db.admin.page.SortSpec` for :attr:`UserFilters.sort` and for
    :func:`~bayram.db.admin.page.decode_sorted_cursor`, and the
    :class:`~bayram.db.admin.page.SortValueKind` that same decoder needs to read the token's
    ...
    Raises :class:`~bayram.db.admin.segment.SegmentError` for a key the registry cannot order
    by, which is unreachable for a sort that came out of
    :func:`~bayram.db.admin.segment.compile_segment` — that is the boundary which turns an
    unknown key into a 422. A sortable field is an instant or an integer and nothing else: a
    text sort key would be a total alphabetical order over account names, the same unmask oracle
    :data:`_SEARCHABLE_COLUMNS` refuses.
    """
...
async def list_users(
    ...
    *,
    ...
    :func:`~bayram.db.admin.page.decode_sorted_cursor` exists to make at the HTTP boundary — and
...
async def count_segment_exactly(session: AsyncSession, *, filters: UserFilters) -> int:
    ...
    **Deliberately not :func:`count_users`.** That one stops at
    :data:`~bayram.db.admin.page.TOTAL_COUNT_CAP` and labels the answer ``is_exact=False``, which
...
async def segment_breakdown(session: AsyncSession, *, filters: UserFilters) -> SegmentBreakdown:
    ...
    **One grouped statement, not five counts.** The three dimensions a preview reports —
    ...
    overlap between :attr:`~bayram.db.admin.views.SegmentBreakdown.blocked` and
    :attr:`~bayram.db.admin.views.SegmentBreakdown.bot_blocked` is a fact about the audience,
...
async def get_user_detail(
    ...
    *,
    ...
    ``BAYRAM_SETTLEMENT_GRACE_S`` or derived from its queue ladder
    (:func:`bayram.entitlements.resolve_entitlement_policy`), so a deployment that moved either
...
async def load_avatar(
    ...
    :class:`~bayram.db.admin.views.UserListItem` here would put a phone number and a name into
...
async def _sorted(
    ...
    *,
    ...
    :class:`~bayram.db.admin.views.UserListItem` — a page that re-derived it in Python would be
    ...
    :func:`~bayram.db.admin.page.sorted_keyset_predicate` requires and what keeps a tie-block from
    being paged twice.
...
def _list_item(
    ...
    :attr:`~bayram.db.admin.views.UserListItem.credit_balance` argues why — the column is
