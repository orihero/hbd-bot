...
async def get_for_live_session(
    ...
    ``WHERE`` clause for the reason ``bayram.db.admin.sessions`` gives: a filter is not a branch
...
async def insert_first_owner(
    ...
    *committed* row, so the read-then-insert window — which spanned two round trips and an
    ...
    it; :mod:`bayram.admin.bootstrap` turns it into one operator-facing line.
    """
    ...
    # ``UPDATE``, which is why ``bayram.db.admin.sessions`` counts ids instead.
