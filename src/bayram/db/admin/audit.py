...
constructs an :class:`~bayram.db.models.admin_audit.AdminAuditRow`. That is not a convention
...
text still passes through ``bayram.logging.redact``.
...
from bayram.logging import get_logger, redact
...
SUBJECT_TYPES: Final[frozenset[str]] = frozenset(
    {
        ...
        "broadcast",
        # ONE payment intent, identified by its ``payment_intents.id`` — a UUID, never
        # ``public_ref`` (which the rail also holds) and never ``idempotency_key`` (which
        # contains the customer's Telegram id). A payment is not an ``order``: the two are
        # joined by nothing this schema records, because a purchased credit is fungible and
        # cannot be attributed to the song it rendered. It is not a ``user`` either — money
        # columns survive ``/forget`` and the buyer does not, so an erased payment still has
        # a subject while its buyer has none. It is filed as its own type so that
        # "everything anyone did to this payment" stays an indexed equality on
...
#: Shapes ``bayram.logging.redact`` does not know about, because they are not things the bot
...
class AuditValueRejectedError(ValueError):
    ...
    Raised rather than returned: this module follows ``bayram.db.admin``'s rule that the request
...
def _checked_ip(value: str | None) -> str | None:
    ...
    ``ip`` reaches this row from :func:`bayram.admin.deps.resolve_request_ip`, which already
