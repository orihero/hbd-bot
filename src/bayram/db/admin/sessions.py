...
``bayram.db.purge``. ``rowcount`` after a bulk ``UPDATE`` is a driver-dependent number, and
...
async def purge_expired_sessions(session: AsyncSession, *, now: datetime, batch_size: int) -> int:
    ...
    Bounded for the same reason every sweep in ``bayram.db.purge`` is: a first run against a
