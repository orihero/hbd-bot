...
The predicates come from :func:`bayram.db.purge.rows_past_expiry_statements`, not from a copy
...
rest of ``bayram.db.admin`` keeps, so a request that writes a run record and then fails to
...
async def record_run(
    ...
    *,
    ...
    the caller knows it: :func:`bayram.db.purge.purge_expired` hands back keys and the job
    ...
    row = PurgeRunRow(
        ...
        vendor_usage_deleted=report.vendor_usage_deleted,
        membership_events_deleted=report.membership_events_deleted,
        chat_bodies_purged=report.chat_bodies_purged,
