...
These aggregates have no home in :mod:`bayram.db.admin.users`, and the split is deliberate
...
:mod:`bayram.db.admin.metrics` holds without exception; nothing here fetches rows and folds
them in Python.
...
three bare :func:`~bayram.db.admin.sql.count_where` cutoffs and no ``WHERE`` at all — has no
...
**``now`` is a parameter, never a call**, per :mod:`bayram.admin.window`'s rule. Routers read
...
from bayram.contracts import BotMembershipEvent, Language
...
async def account_totals(
    ...
    **It must never route through** :func:`~bayram.db.admin.page.bounded_total`. That helper
...
async def new_accounts(session: AsyncSession, *, window: TimeWindow | None = None) -> Trend:
    ...
    One index range scan for both numbers — see :func:`~bayram.db.admin.sql.count_pair`.
...
async def language_mix(
    ...
    :data:`~bayram.db.users_sql.DEFAULT_UI_LANGUAGE` is pinned to, so every account lands in
    ...
    **A language no account holds is ABSENT, and the reason is presentational rather than
    ...
    bar like :func:`~bayram.db.admin.orders.order_state_totals`, which zero-fills precisely so
    its segments do not re-lay-out under the operator's cursor. A caller that wants fixed
    slots can fill them from :class:`~bayram.contracts.Language` itself, because
    :attr:`~bayram.db.admin.views.LanguageMixTotals.accounts` travels with the entries and a
    ...
    **``window`` narrows by ``users.created_at``** — the mix among accounts that SIGNED UP in
    ...
    per :class:`~bayram.contracts.Language` member, four of them today — which is the same
    licence :func:`~bayram.db.admin.metrics.failure_breakdown` takes for its shares. It is not a
    ...
    :func:`~bayram.db.base.enum_type` builds a ``VARCHAR`` and never a native enum — so two
...
async def activity_history(
    ...
    **The window is applied to ``taken_at``, not to ``snapshot_date``**, though the latter is
    the indexed column. A :class:`~bayram.db.admin.sql.TimeWindow` bound is an instant, and
    comparing one to a ``DATE`` means a cast that Postgres resolves through the *session*
    ``TimeZone`` — the identical trap :class:`~bayram.db.admin.sql.UtcDay` exists to close, and
...
async def has_recorded_churn(session: AsyncSession) -> bool:
    ...
    argument :func:`~bayram.db.admin.vendor_usage.has_recorded_vendor_usage` makes.
    """
