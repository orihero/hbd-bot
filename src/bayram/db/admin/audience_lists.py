...
``bayram.admin.routers.reveal`` may be weakened on the strength of this module existing. The
...
time-based sweep in :mod:`bayram.db.purge`, with ``purge_user`` planned and unwritten
...
from bayram.db.admin.page import keyset_order
...
async def top_generators(
    ...
    **The ranking key is deliveries, not orders and not money.** An account that opens
    ...
    :attr:`~bayram.db.admin.views.TopGenerator.orders_created` is published beside the ranking
...
async def recent_subscribers(
    ...
    **``is_stub_rail`` travels with every row.** ``StubCheckoutProvider`` stamps
    ...
    bayram.checkout.STUB_PROVIDER_NAME``, so no later layer has to know the constant to stay
    ...
    **An expired plan with songs left is BREAKAGE; a running one is an OBLIGATION.**
    ...
    :func:`~bayram.db.admin.page.keyset_order` — reused rather than respelled even though this
