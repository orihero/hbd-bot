...
These live beside :mod:`bayram.db.purge` rather than inside it for two reasons: that module is
already near the repo's file ceiling, and these three sweeps are the only ones that have to
know about Postgres privileges. Everything here is called from ``bayram.db.purge._purge``, in
...
leaves a shorter chain that verifies end to end. :func:`bayram.db.admin.audit.verify_chain`
checks both.
"""
...
from bayram.db.admin import sessions as session_store
...
async def purge_audit_reasons(session: AsyncSession, *, now: datetime, limit: int) -> int:
    ...
    :mod:`bayram.db.models.admin_audit`). ``reason_purged_at`` is stamped so "empty because
...
async def pin_audit_head(session: AsyncSession, *, now: datetime) -> None:
    ...
    pins is in the log line :func:`bayram.db.admin.audit.write_head_anchor` emits for every
