...
Two shapes live here on purpose, and the split is the same one ``bayram.db.admin`` makes:
...
The statements themselves live in :mod:`bayram.db.credit_sql`, the two-statement erasure
``/forget`` runs in :mod:`bayram.db.credit_erasure` (its asymmetry — delete the balance, keep
the count anonymous — is argued there), and everything that CLOSES a debit — ``settle``,
``refund``, ``consume`` and the hourly sweep — is in :mod:`bayram.db.credit_settlement`, split
...
from bayram.db.credit_settlement import (
    SWEEP_ACTOR,
    consume,
    refund,
    settle,
...
from bayram.db.plan_sql import claim_plan_song, live_plan
...
#: ``settle``, ``refund``, ``consume`` and the sweep live in :mod:`bayram.db.credit_settlement`
...
async def _mint_plan_song(
    ...
    :func:`bayram.db.plan_sql.claim_plan_song` does an optimistic
    ...
    **Nothing here expires and nothing is ever clawed back.** When ``plan_ends_at`` passes,
    :func:`bayram.db.plan_sql.live_plan` stops returning the row and the remaining songs are
    ...
    minting is lazy; :mod:`bayram.db.plan_sql` argues it at length.
    """
...
class SqlCreditLedger:
    ...
    async def _forget(self, telegram_user_id: int) -> None:
        ...
        :class:`bayram.db.credit_erasure.CreditErasure` reports and this line drops would make
