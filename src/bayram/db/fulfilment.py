...
Written to the same rule as :mod:`bayram.db.credit_sql`, :mod:`bayram.db.plan_sql` and
:mod:`bayram.db.topup_sql` one level up: the session comes FIRST and POSITIONAL, it is NOT
...
are pure reads of a frozen :class:`bayram.checkout.Purchase`, neither writes anything before it
...
:func:`bayram.db.credit_sql.read_balance` projects a due allowance and a live plan's unminted
songs, and it can only do that against the :class:`bayram.entitlements.EntitlementPolicy` the
...
**Nothing here is wrapped in :func:`bayram.db.guard.run_guarded` and nothing here returns
...
:func:`write_plan_sale` returns a ``PlanPurchaseRow``. Rule 15 governs the ``bayram.db`` PACKAGE
boundary, not this file, and a mapped row may move freely within it; the two callers want
different things out of that row — the bot turns it into a :class:`bayram.checkout.PlanState`
...
from bayram.db.enums import CreditReason, PlanKind, TopupKind
from bayram.db.models.plan_purchase import PlanPurchaseRow
from bayram.db.plan_sql import current_plan, insert_plan, plan_by_key
from bayram.db.topup_sql import insert_topup
...
#: into two and make "how many songs were bought?" a UNION. ``bayram.db.purchases`` re-exports it
...
async def write_single_sale(
    ...
    *,
    ...
    Calls the MODULE-LEVEL :func:`bayram.db.credits.grant`, which accepts ``reason=`` even
    though ``EntitlementStore.grant`` does not — that protocol speaks no database enum
    (``bayram.entitlements`` argues why it must not), so the only way a purchase can be
    ...
    **The receipt is written FIRST, deliberately.** Both writes are in one transaction,
    ...
    :func:`bayram.db.credit_erasure.forget_account` makes for receipts-first, balance-last.
...
async def write_plan_sale(
    ...
    **One plan at a time**, decided by :func:`bayram.db.plan_sql.current_plan`, which ignores
    ...
    two predicates on :class:`bayram.checkout.PlanState` are named separately.
    ...
    the caller's transaction, which is why it may not leave ``bayram.db``.
    """
    _refuse_an_unpaid_purchase(purchase, telegram_user_id=telegram_user_id)
...
def _refuse_an_unpaid_purchase(purchase: Purchase, *, telegram_user_id: int) -> None:
    """Nothing is written for a :class:`bayram.checkout.Purchase` that is not paid.
    ...
    rail — which is what Payme actually is — returns an UNPAID :class:`bayram.checkout.Purchase`
...
def _refuse_a_product_that_is_not_a_topup(purchase: Purchase, *, telegram_user_id: int) -> None:
    ...
    :class:`bayram.checkout.Product` is the catalogue and holds ``STARTER`` as well as
    ``SINGLE``; :class:`bayram.db.enums.TopupKind` deliberately mirrors only the one-off half,
    ...
    :func:`bayram.db.guard.run_guarded` catches ``BayramError``, ``IntegrityError``,
...
def _refuse_a_product_that_is_not_a_plan(purchase: Purchase, *, telegram_user_id: int) -> None:
    ...
    :class:`bayram.checkout.Product`, so a ``SINGLE`` purchase could not reach it. That
