...
The exact sibling of :mod:`bayram.db.topup_sql` and :mod:`bayram.db.plan_sql`, written to the same
...
that sequences them lives in :mod:`bayram.db.payme`, so that "what does a settlement write?"
...
:func:`bayram.db.credit_sql.debit_balance` is ``… SET balance = balance - :cost WHERE balance >=
:cost``, and :func:`bayram.db.plan_sql.claim_plan_song` is ``… SET songs_used = songs_used + 1
...
performing it. :func:`bayram.db.credit_sql.account_state` avoids the identical trap by selecting
...
These names are public to the ``bayram.db`` package and to nothing else (Rule 15): a caller
outside persistence has no business holding a session, which is exactly why
:mod:`bayram.payme.ports` declares a protocol of ``Result``-returning coroutines instead.
...
async def intent_by_id(session: AsyncSession, intent_id: UUID) -> PaymentIntentRow | None:
    ...
    the stale read the whole module is written to avoid. :func:`bayram.db.credit_sql.account_state`
...
async def transactions_in_period(
    ...
    **A join rather than a transaction read followed by N intent reads.** A statement row IS
    ...
    deletes an intent that has a transaction on it (``bayram.db.purge`` sweeps terminal intents
...
async def claim_intent_for_operator(
    ...
    The recovery button behind ``bayram.payme.cli settle``, for the incident this rail cannot
...
async def mark_performed(session: AsyncSession, *, transaction_id: UUID, now: datetime) -> bool:
    ...
    rowcount is a real illegal transition. See :meth:`bayram.db.payme.SqlPaymeLedger.perform`.
...
async def mark_cancelled(
    ...
    *,
    ...
    ours — argued at length on :class:`bayram.db.models.payme_transaction.PaymeTransactionRow`.
...
async def anonymise_intents(session: AsyncSession, *, telegram_user_id: int) -> int:
    ...
    :func:`bayram.db.topup_sql.anonymise_topups` and :func:`bayram.db.plan_sql.anonymise_plans`.
    It is the replay marker: rewriting it would let a second purchase under the same key open a
    second intent for a customer who has asked to be forgotten — ``/forget`` as a way of
    clearing your own deduplication memory.

    **:func:`bayram.db.credit_erasure.forget_account` spells this statement out itself rather than
    calling this function**, and that duplication is deliberate on its side: erasure must keep
    working, and keep being asserted by test, on a deployment where ``BAYRAM_CHECKOUT_PROVIDER``
