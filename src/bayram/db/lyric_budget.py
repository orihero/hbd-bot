...
The shape is the split :mod:`bayram.db.credits` makes and for the same reasons:
...
``bayram.db.credit_sql.account_state``: a second ``SELECT`` inside the same transaction is
...
class SqlLyricBudget:
    """:class:`bayram.lyric_budget.LyricBudgetStore` over the ``lyric_budgets`` table.
    ...
    credit (``bayram.bot.deps.BotDeps.entitlements``), and a second write on that protocol is
