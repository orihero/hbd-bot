...
that a database, an order, or a pipeline exists. It imports ``bayram.contracts``,
``bayram.logging`` and the standard library and **nothing else** — deliberately not
``bayram.db`` and not ``bayram.pipeline`` — so every layer in the system is free to depend on
...
``bayram.logging.correlation_scope`` binds a correlation id, so not one provider method grows
...
persist. Implementations swallow, log and return; see ``bayram.db.vendor_usage.DbUsageSink``.
"""
...
#:
#: Distinct from ``bayram.providers.music.usage.USAGE_EVENT`` (``"music.usage"``) on purpose:
...
class VendorUsage:
    ...
    #: True under ``BAYRAM_USE_FAKE_PROVIDERS``. Recorded rather than skipped, so a demo run is
    #: visibly excluded instead of silently absent.
    is_fake: bool = False
    http_status: int | None = None
    #: A member of the closed ``bayram.errors`` taxonomy. NEVER a response-body excerpt: a
...
_USAGE_CONTEXT: ContextVar[UsageContext] = ContextVar("bayram_usage_context", default=_UNBOUND)
...
def usage_scope(
    *, order_id: UUID | None = None, task: UsageTask | None = None
    ...
    Mirrors ``bayram.logging.correlation_scope``'s set/reset-token shape, and nests the same
...
class LoggingUsageSink:
    ...
    database — a unit test, a one-shot operator tool, the ``bayram.demo`` runner — still gets
    the measurement on stdout and nothing has to be wired for the code to be correct.
    ``bayram.runtime.providers`` replaces it with the persisting sink in the real processes.
    """
