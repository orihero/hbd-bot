...
#: comes from ``Settings`` through :class:`~bayram.bot.pricing.Pricing` rather than from the
#: catalogues, because a price written into four catalogues would silently disagree with
#: ``BAYRAM_SINGLE_SONG_PRICE_MINOR`` the day an operator changed it — the customer would be
...
def checkout_link_keyboard(language: Language, url: str) -> InlineKeyboardMarkup:
    ...
    the first of its kind here: before this builder, ``grep -rn "url=" src/bayram/bot/`` returned
    nothing at all.
