...
from bayram.bot.handlers.balance import show_confirm
...
from bayram.bot.lyrics_entry import parse_typed_lyrics
from bayram.bot.screens import Screen
from bayram.bot.states import Wizard, WizardStep, next_step, state_for
from bayram.contracts import Err, LyricDraft, Result, UsageTask
from bayram.logging import get_logger
from bayram.lyric_budget import LyricBudgetVerdict
...
async def _has_daily_budget(
    ...
    **Fails OPEN on an unreadable counter**, logged at ERROR. Every customer-facing gate in
    ...
    read, ``bot.gate``'s block check, ``bayram.ratelimit``'s throttle — and here the exposure it
...
async def _refuse_for_today(
    ...
    ``used`` can therefore exceed it — see :meth:`bayram.lyric_budget.LyricBudgetStore`.
    """
    language = draft.ui_language
    text = translate(
...
async def handle_lyrics_ok(callback: CallbackQuery, state: FSMContext, deps: BotDeps) -> None:
    ...
    later and it writes nothing — see :mod:`bayram.bot.handlers.balance`.
    """
