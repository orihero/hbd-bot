...
whenever the FSM cache is cold, inside that lock. :data:`~bayram.bot.draft.ONBOARDED_KEY`
...
from bayram.bot.avatar import fetch_and_store_avatar
from bayram.bot.callbacks import LanguageCB, LanguageSlot
...
#: What is written into :data:`~bayram.bot.draft.ONBOARDED_KEY`, and the one direction of this
...
def _language_from(data: dict[str, object]) -> Language | None:
    ...
    :data:`~bayram.bot.draft.UI_LANGUAGE_KEY` rather than a call into it, and the reason is that
    ...
    Anything that is not a string naming a real :class:`~bayram.contracts.Language` member is
...
async def load_identity(state: FSMContext, deps: BotDeps, telegram_user_id: int | None) -> Identity:
    ...
    **The cache write is why this is not a pure function.** Every update that gets past
    ...
    the only place that can repair :data:`~bayram.bot.draft.UI_LANGUAGE_KEY` after something
    outside this process wipes the FSM dict. ``bayram.runtime.jobs`` does exactly that when a
    ...
    **The language write-back is NON-CLOBBERING**, and that guard is load-bearing two files
    ...
    :data:`~bayram.bot.draft.UI_LANGUAGE_KEY` is written only when the key is ABSENT, and the
...
async def handle_contact_shared(
    ...
    **An ``Err`` from ``record_contact`` is the one store failure in this module that must not
    ...
    is on their critical path — see :func:`~bayram.bot.avatar.fetch_and_store_avatar`.
    """
