...
from bayram.bot.handlers.balance import build_offer, show_confirm
...
from bayram.bot.handlers.lyrics import enter_lyrics_step
...
from bayram.contracts import Brief, Err, Language, Order, OrderState
...
from bayram.logging import current_correlation_id, get_logger, new_correlation_id
...
#: of the seam may not write. It mirrors the ``cost`` default of ``bayram.db.credits.charge``;
...
async def _authorize_and_submit(
    ...
    from ``bayram.main``, never the credit-gated decorator, so the three early returns below
...
async def _entitlement_refusal(deps: BotDeps, order: Order) -> BayramError | None:
    ...
    comment: the only call made is :meth:`~bayram.entitlements.EntitlementStore.balance_for`,
    ...
    **A read that FAILS now depends on whether this deployment SELLS, and that is a change
    ...
    so ``bayram.db.credits.charge`` covers the shortfall with an ``UNENFORCED_RENDER`` grant and
    ...
    The return type widened to ``BayramError`` for that sentence. :func:`_refusal_for` still
...
def _refusal_for(
    ...
    sets ``BAYRAM_CREDITS_ENFORCED=true``. One boolean, one branch.

    That boolean is read off the RESOLVED policy rather than off ``Settings`` directly, so
    this screen and ``bayram.db.credits.charge`` cannot end up disagreeing about what "dark"
...
def _insufficient_credits(
    ...
    :func:`bayram.entitlements.period_index_for`), so the next one opens at the start of the
...
def _refusal_text(error: BayramError, deps: BotDeps, language: Language) -> str:
