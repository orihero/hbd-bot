...
from bayram.bot.draft import UI_LANGUAGE_KEY, load_draft
from bayram.bot.i18n import FALLBACK_LANGUAGE, translate
from bayram.contracts import Language, is_ok
from bayram.errors import GENERIC_USER_MESSAGE_KEY, BayramError
from bayram.logging import correlation_scope, current_correlation_id, get_logger
...
def _language_or_none(raw: object) -> Language | None:
    ...
    Deliberately NOT :func:`~bayram.bot.i18n.parse_language`, which answers
    :data:`~bayram.bot.i18n.FALLBACK_LANGUAGE` for an unknown code. Borrowing it here would
...
async def resolve_language_or_none(state: FSMContext | None) -> Language | None:
    ...
    ``None`` rather than :data:`~bayram.bot.i18n.FALLBACK_LANGUAGE` is the reason this function
...
class ErrorGuardMiddleware(BaseMiddleware):
    ...
    async def _report(self, event: TelegramObject, data: dict[str, Any], exc: Exception) -> None:
        ...
        key = exc.user_message_key if isinstance(exc, BayramError) else GENERIC_USER_MESSAGE_KEY
        _LOG.error(
            "handler failed",
            extra={
