"""The ARQ worker process. ``python -m arq bayram.worker.WorkerSettings``.
...
process (``bayram-payme.service``) holding exactly one credential — the cashbox key — and no
...
:func:`bayram.runtime.payme_jobs.notify_payment_settled` and this process, which already holds a
