...
**And why this module does not use** :mod:`bayram.admin.audit_sink`. That module's contract is
...
from bayram.admin.deps import CurrentAdmin
from bayram.admin.errors import AdminErrorCode, AdminProblem, ProblemError, unwrap
...
#: field added to :class:`~bayram.admin.schemas.reveal.RevealField` with no source here raises
...
async def read_records(db: AsyncSession, plan: RevealPlan) -> RevealResult:
    ...
    **The subject is tested before the shape, and the order is load-bearing.** Every
    ...
    Reads the columns directly rather than going through :mod:`bayram.admin.schemas.orders`:
