...
``BAYRAM_ELEVENLABS_API_KEY`` — there is no field — so the *value* is already unreachable; this
...
``app`` is built at import so ``uvicorn bayram.admin.app:app`` works, and ``create_app()``
...
decisions this module makes, and the vendor list is derived from ``bayram.config``; every other
...
from bayram.admin.deps import API_PREFIX
from bayram.admin.errors import handle_unexpected, install_error_handlers
from bayram.admin.logging_mw import RequestLogMiddleware
from bayram.admin.middleware import (
    CorrelationIdMiddleware,
    SecurityHeadersMiddleware,
    UnhandledErrorMiddleware,
)
from bayram.admin.middleware.security_headers import (
    IMMUTABLE_PATH_PREFIX,
    STYLE_NONCE_STATE_KEY,
)
from bayram.admin.routers import (
...
from bayram.admin.shell import render_shell
...
def derive_forbidden_env_vars() -> tuple[str, ...]:
    """The variable names this host must not have within reach, derived from ``bayram.config``.

    **Two populations, and they are different kinds of thing.** The first is every credential
    declared as a FIELD on :class:`bayram.config.Settings` — ``BAYRAM_TELEGRAM_BOT_TOKEN``,
    ``BAYRAM_ELEVENLABS_API_KEY``, ``BAYRAM_LLM_API_KEY``, ``BAYRAM_LLM_FALLBACK_API_KEY``,
    ``BAYRAM_OPENROUTER_MANAGEMENT_KEY`` — spelled by upper-casing ``VENDOR_SECRET_FIELDS``, so
    a sixth credential added to that class is covered here without anyone remembering this file
    exists. The second is ``FOREIGN_SECRET_ENV_VARS``: environment variable NAMES belonging to a
    settings model this process never loads at all (``bayram.payme.settings.PaymeSettings``),
    ...
    ``BAYRAM_PAYME_MERCHANT_KEY`` is in that second population, and it is the mechanism that
    ...
    process holding it, recorded in full at ``bayram.config.FOREIGN_SECRET_ENV_VARS``.
...
#: element at runtime. See :mod:`bayram.admin.shell` for why that is not optional and why the
...
def _admin_env_file() -> str:
    """The dotenv file this process actually reads — ``.env.admin`` or ``BAYRAM_ADMIN_ENV_FILE``.
...
def _env_file_names() -> frozenset[str]:
    ...
    ``bayram_llm_api_key`` line is just as live as the shouted form.
    """
...
async def _serve_spa_index(request: Request) -> Response:
    ...
    **A missing bundle must 404, not 500.** ``src/bayram/admin/static/`` is gitignored, so in a
    ...
    disk, because it carries this response's style nonce (:mod:`bayram.admin.shell`). That is
    ...
    async def lifespan(application: FastAPI) -> AsyncIterator[None]:
        ...
        _LOGGER.info(
            ...
            extra={
                ...
                # BAYRAM_ADMIN_ENV_FILE can boot a prod panel from a dev checkout.
...
def create_app(
    ...
    # ``billing.py`` ships FOUR routers, and every split is a permission cell rather than a
    # filing choice. The six rail reads are aggregate counts, closed enums, provider names and
    # probe booleans — DASHBOARD_READ, the same cell the metrics beside them stand on, and
    # ``/api/billing/calls`` is among them because ``payme_rpc_log`` holds no Telegram id, no
    # body and no header at all. The three payment reads publish a MASKED buyer, so they stand
    # on RECORDS_READ beside every other record this panel lists. Pause and resume stand on
    # RAIL_CONTROL (ADMIN and OWNER, no step-up — argued beside the permission), and the one
    # notify stands on PAYMENT_NOTIFY, which reaches SUPPORT precisely because SUPPORT takes
    # the call it answers. The guard is per-router (§12.1 T3), so each needs its own line, and
    # mounting three of the four would drop the fourth with no error anywhere.
    application.include_router(build_rail_router())
    application.include_router(build_billing_records_router())
    application.include_router(build_rail_control_router())
    application.include_router(build_payment_notify_router())
