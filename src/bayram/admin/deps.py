...
statement: :func:`bayram.db.admin.accounts.get_for_live_session` joins the two tables, so a
...
The Redis mirror (:mod:`bayram.admin.sessions`) therefore holds only the session row, and even
...
from bayram.admin.csrf import SESSION_COOKIE_NAME, CsrfDecision, verify_csrf_request
...
from bayram.admin.security.clientip import resolve_client_ip
...
from bayram.admin.sessions import SessionSnapshot, invalidate_mirror, resolve_session, touch_session
...
# ---------------------------------------------------------------------------
# Both live here rather than in :mod:`bayram.admin.security` for one reason: they need
