...
configuration layer, so a typo in ``BAYRAM_ADMIN_TRUSTED_PROXY_CIDRS`` fails the boot rather
...
def parse_trusted_proxies(cidrs: Sequence[str]) -> tuple[IpNetwork, ...]:
    ...
    for raw in cidrs:
        ...
        try:
            ...
            raise ConfigError(
                f"BAYRAM_ADMIN_TRUSTED_PROXY_CIDRS contains an unparsable entry: {candidate!r}",
