...
:class:`~bayram.admin.settings.AdminSettings`: the field lands in ``model_dump()`` the moment
...
:class:`~bayram.admin.schemas.common.ApiModel`, so a number on the panel maps to the
``BAYRAM_ADMIN_*`` variable an operator would edit without a translation table. The four
...
carries a tier — the ``BAYRAM_ADMIN_*`` settings are not on the bot's frozen
``bayram.config.Settings`` at all, so there is no tiered field for an override to shadow. The
