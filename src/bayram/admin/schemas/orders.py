...
from bayram.admin.serializers.redaction import mask_name, mask_telegram_user_id
from bayram.admin.serializers.retryability import is_retryable_code
from bayram.admin.serializers.stage_plan import (
    GreetingEvidence,
    StageOutcome,
    StagePlan,
    build_stage_plan,
...
class OrderView(ApiModel):
    ...
    #: :class:`~bayram.db.admin.views.OrderLedgerStatus`, which defines all four in ledger
    ...
    #: :class:`~bayram.db.admin.views.OrderPaymentRail` argues both refusals in full.
...
class OrderStateCountsView(ApiModel):
    ...
    Every :class:`~bayram.contracts.OrderState` is present, in enum declaration order, with a
    ...
    :data:`~bayram.db.admin.page.TOTAL_COUNT_CAP`-bounded the way ``meta.total`` on the list is.
