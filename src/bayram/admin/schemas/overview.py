...
**Week and month are folded here, not grouped in SQL.** :class:`~bayram.db.admin.sql.SeriesGrain`
...
than reveal-gated; those shapes live in :mod:`bayram.admin.schemas.audience_lists`, are served
...
from bayram.admin.schemas.vendors import cost_source_of
...
#: moment ``BAYRAM_ADMIN_DASHBOARD_RUN_RATE_DAYS`` is anything but thirty, and silently so. The
...
class AbsenceReason(StrEnum):
    ...
    #: No FX rate is published on this deployment. Set ``BAYRAM_ADMIN_UZS_PER_USD`` and its date.
    NO_FX_RATE = "no_fx_rate"
    #: No unit price is mirrored into the panel. Set ``BAYRAM_ADMIN_SINGLE_SONG_PRICE_MINOR``.
...
class SeriesBucket(StrEnum):
    ...
    :class:`~bayram.db.admin.sql.SeriesGrain` for why they are not SQL.
    """
...
class UsdCost(ApiModel):
    ...
    #: member — see :mod:`bayram.admin.schemas.vendors`.
...
class SubscriptionChurnView(ApiModel):
    ...
    **Read ``endedPlans`` before reading ``rate``.** One thirty-day product first sold weeks
    ...
    #: back when the truth is that nobody can tell. :class:`~bayram.db.admin.views.
    #: SubscriptionChurn` states the same partition at the view and
    #: :func:`~bayram.db.admin.plan_purchases.subscription_churn` at the query, so the three
...
class ComponentStatusView(ApiModel):
    ...
    **No outbound call produces any of these.** They are read from
    :func:`~bayram.db.admin.metrics.read_capabilities` and from the cached balance rows the
...
class PerformanceResponse(ApiModel):
    ...
    #: :func:`~bayram.db.admin.metrics.delivered_latency`. NOT the created-at cohort the pulse
...
class CostProvenanceView(ApiModel):
    ...
    **"0 reported" and "not priced" must never render the same.** A vendor that reports a
    ...
    #: The real :class:`~bayram.contracts.CostSource` member, not the ``str`` the collapsed
...
class PlanUtilisationBucketView(ApiModel):
    ...
    :class:`~bayram.admin.schemas.dashboard.SimilarityBucketView` already put on the wire for
...
def _bucket_axis(bucket: SeriesBucket, window: TimeWindow) -> list[tuple[str, datetime]]:
    ...
    Half-open on the upper end, matching :class:`~bayram.db.admin.sql.TimeWindow`, so the
...
def _to_currency_amount(amount: CurrencyAmount) -> CurrencyAmountView:
    """The single conversion point for :class:`~bayram.db.admin.views.CurrencyAmount`."""
