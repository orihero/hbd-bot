...
that reached a TERMINAL state, which :attr:`~bayram.db.admin.views.DeliveryOutcome.success_rate`
...
layer does not fill it in, even though ``bayram.db.admin.metrics`` says the API layer knows the
...
class FailureView(ApiModel):
    ...
    count: int
    share: float
    #: Tri-state. ``null`` means no class in ``bayram.errors`` claims this code, so the panel
    #: says "unknown" instead of offering a retry that cannot help — see
    #: :mod:`bayram.admin.serializers.retryability` on why this is a hint about the class of
...
class WindowView(ApiModel):
    ...
    **``from`` is nullable and ``to`` is not**, and the asymmetry is the whole contract of a
    ...
    (:func:`bayram.admin.window.resolve_window`), and echoing it is what keeps the chart
...
class CapabilitiesView(ApiModel):
    ...
    Named one-to-one with :class:`~bayram.db.admin.views.ReadCapabilities`. A capability
