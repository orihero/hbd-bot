...
This module therefore walks :class:`BayramError`'s subclass tree once, at import, and builds
``code → default_is_retryable``. A new error class added to ``bayram.errors`` next quarter is
...
def _walk(root: type[BayramError], claims: dict[ErrorCode, set[bool]]) -> None:
