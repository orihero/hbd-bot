...
(:func:`~bayram.admin.window.resolve_window`): ``?from=`` is counted up to the instant the
...
(§6.1), typed as :class:`~bayram.contracts.Vendor` so an unknown value is a 422 from FastAPI
...
:mod:`bayram.db.admin.vendor_usage`), so a ``BAYRAM_USE_FAKE_PROVIDERS`` demo no longer inflates
...
def build_vendors(
    ...
    The asymmetry :func:`~bayram.db.admin.sql.apply_in` documents, made explicit at the edge:
...
def build_include_fake(
    ...
    """``?includeFake=`` — whether a ``BAYRAM_USE_FAKE_PROVIDERS`` run is counted. Default false.
...
def build_exclude_health(
    ...
    :data:`~bayram.db.admin.vendor_usage.SPENDABLE_EXCLUSIONS` in their own bodies rather than
