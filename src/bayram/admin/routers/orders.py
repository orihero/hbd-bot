...
**Nothing here reaches past :mod:`bayram.db.admin.orders`, and that is the entire reason this
module is thin.** ``bayram.db.mapping.to_order`` delegates to ``to_brief`` →
...
**Masking is not a decision made here.** ``bayram.admin.schemas.orders`` has one projection per
...
from bayram.admin.schemas.orders import (
    AssetsPage,
    AttemptsPage,
    OrderDetailView,
    OrdersPage,
...
from bayram.db.admin.assets import AssetFilters, count_assets, list_assets
from bayram.db.admin.attempts import AttemptFilters, count_attempts, list_attempts
from bayram.db.admin.orders import (
    OrderFilters,
    count_orders,
...
from bayram.db.models.order import CORRELATION_ID_LENGTH
...
def _window(since: datetime | None, until: datetime | None) -> TimeWindow | None:
    ...
    :func:`~bayram.admin.window.resolve_window` owns every judgement — awareness, which bound is
...
def build_query(
    ...
    ``q`` declares ``max_length`` even though :func:`~bayram.db.admin.sql.search_clause`
