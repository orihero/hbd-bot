...
honestly. This module is the port that closes that; ``bayram.db.churn.SqlBotBlocks`` is the
implementation and ``bayram.bot.handlers.membership`` plus ``bayram.runtime.jobs`` are the two
callers.

**Why this is not a method on ``EntitlementStore``.** That seam answers "may this account
render?", every gate in the bot holds it, and ``bayram.bot.deps.BotDeps.entitlements`` is
documented as read-only-for-gates precisely because a gate that could DEBIT could
double-charge. :mod:`bayram.lyric_budget` makes the identical argument for the identical
...
**Why this module is a LEAF.** It imports ``bayram.contracts`` and nothing else — never
``bayram.db``, never ``bayram.config``. A protocol that named ``bayram.db.enums`` would drag the
whole ``bayram.db`` package into every importer, which is the import cycle
``bayram.entitlements``' docstring already records; and both callers are outside ``bayram.db``
...
``BotBlockSource`` are declared in :mod:`bayram.contracts` beside ``Vendor`` and ``CostSource``
...
``bayram.contracts`` and the modules under ``bayram.db`` for enums whose members must fit their
column, so an enum declared here would be silently exempt from the check that keeps a
member from being too long for its ``VARCHAR(32)``. And ``bayram.db.enums``' own rule ("nothing
...
import the two enums from ``bayram.contracts`` and the protocol from here; this module
...
from bayram.contracts import BotBlockSource, Result
...
class BotBlockRecorder(Protocol):
    ...
    ``bayram.db.credit_sql.insert_or_ignore`` documents, and it is what lets the two callers
