...
This is what ``BAYRAM_USE_FAKE_PROVIDERS=1`` installs, and it exists so the whole application
...
That matters: two packages define lyric payloads (``bayram.pipeline.content`` and
``bayram.providers.llm.schemas``) and this file must not import either, or the fake would drag
...
from bayram.errors import BayramError, LlmParseError
...
class FakeLlmProvider:
    ...
    async def _record(
        self, *, operation: VendorOperation, error: BayramError | None, latency_ms: int
    ) -> None:
