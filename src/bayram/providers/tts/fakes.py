...
A ``BAYRAM_USE_FAKE_PROVIDERS`` run that recorded nothing would look exactly like a deployment
...
from bayram.providers.music.fake import silent_mp3
from bayram.providers.tts.markup import apply_name
from bayram.providers.tts.metering import estimate_speech_duration_s
...
FAKE_AUDIO_MAGIC: Final[bytes] = b"BAYRAMFAKE1"
...
class _Failing:
    ...
    def __init__(self, error: BayramError | None, *, is_persistent: bool) -> None:
        self._error = error
        self._is_persistent = is_persistent
        self._served = 0

    def next_error(self) -> BayramError | None:
