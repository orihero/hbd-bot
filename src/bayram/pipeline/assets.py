...
``bayram.watermark`` and neither touches the ``LyricDraft`` it is handed — a watermark inside
...
from bayram.audio.cover import COVER_MIME, COVER_SUFFIX, render_cover
...
from bayram.errors import AudioProcessingError, BayramError, StorageError
from bayram.logging import get_logger
from bayram.storage import archive_key
from bayram.watermark import SHEET_RULE, audio_tags
...
async def song_asset(
    ...
    *,
    ...
    #: three watermark tags are fixed and come from ``bayram.watermark.audio_tags``.
    title: str,
...
def render_lyric_sheet(lyrics: LyricDraft) -> str:
    ...
    **This function renders a SHEET, and the ``LyricDraft`` it reads is returned to nobody
    ...
    "Generate yours at @bayram_uzbot" as a line of the song, and the model would set it to
...
def storage_key(order_id: object, asset: GeneratedAsset) -> str:
    ...
    Delegates to :func:`bayram.storage.archive_key` rather than spelling the key inline: the
