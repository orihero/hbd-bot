...
check, and the decision to retry is read off ``BayramError.is_retryable`` rather than guessed
...
from bayram.errors import BayramError, NameVerificationExhaustedError, PipelineError
from bayram.logging import correlation_scope, get_logger
from bayram.pipeline.assembly import assemble_kit, persist_kit, validate_brief
...
from bayram.pipeline.moderation import LlmModerator
from bayram.pipeline.name_stage import SongRender, render_song
from bayram.pipeline.outcome import PipelineGap, PipelineOutcome, RunLedger
from bayram.pipeline.personas import select_voices
from bayram.pipeline.plan_builder import build_composition_plan, derive_seed
...
# lives in ``bayram.contracts`` precisely so that a provider adapter stamping a row never has
# to import ``bayram.pipeline``. Mapping them here, once, is what keeps that true.
#
...
# ``BayramError.context`` is built for the LOG, where redaction happens downstream, and
...
# ``reason``; ``bayram.names.resolve`` puts the typed name under ``raw``.
#
...
def _rendered_failed_reason(error: BayramError, *, stage: PipelineStage | None) -> str:
...
def _safe_pairs(error: BayramError) -> list[tuple[str, str]]:
...
def _failed_reason(error: BayramError, *, stage: PipelineStage | None) -> str:
...
class KitPipeline:
    ...
    async def _gated(
        ...
        (``bayram.contracts``:167) and AUTHORIZED is the *tax-side* latch:
    ...
    async def _deliver(self, order: Order) -> None:
        ...
        The DELIVERING frames belong to ``bayram.runtime.jobs._send_kit``, which brackets the
    ...
    async def _fail(
        self, order: Order, reporter: ProgressReporter, error: BayramError, ledger: RunLedger
