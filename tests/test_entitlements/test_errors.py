...
* **The refusals are terminal.** ``Err.is_retryable`` drives the ARQ retry ladder in
  ``hbd.runtime.jobs``, so a retryable "you are out of credits" would re-run the whole
...
* **``hbd.entitlements`` imports nothing but ``hbd.contracts`` and ``hbd.errors``.** Putting
  these types in ``contracts`` reproduced a real circular import; the leaf property is what
  lets ``hbd.db`` depend on this module, and it is asserted rather than assumed.
"""
...
import hbd.entitlements as entitlements
...
from hbd.errors import ConfigError, ErrorCode, HbdError, ValidationError

#: The only first-party imports this module may ever have. See the module docstring.
_ALLOWED_FIRST_PARTY = {"hbd.contracts", "hbd.errors"}
...
def test_every_entitlement_refusal_is_terminal_and_carries_its_own_locale_key(
    error: HbdError, code: ErrorCode, key: str
) -> None:
...
def test_a_naive_clock_is_rejected_rather_than_assumed_to_be_utc() -> None:
    ...
    # second allowance. A ValidationError (an HbdError) is raised rather than a bare
...
def test_the_entitlements_module_imports_nothing_but_contracts_and_errors() -> None:
    # Arrange — putting these types in hbd.contracts reproduced a real cycle
    # (contracts -> hbd.db.enums -> hbd/db/__init__ -> hbd.db.attempts -> contracts). This
    # module is what hbd.db imports, so a first-party import added here re-opens it. The
