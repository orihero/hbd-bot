"""``hbd.checkout`` is a LEAF, and this file is what keeps that true against a helpful edit.

The module's own docstring makes a claim — "it may never import ``hbd.db``" — and the claim
is load-bearing rather than aesthetic. ``hbd.db`` imports ``hbd.checkout`` for ``Purchase``,
``PlanState`` and now :class:`hbd.checkout.PaymentIntent`; a single import in the other
...
argument that keeps ``hbd.contracts`` free of the entitlement types applies here verbatim.

The rail package is barred for a different and stronger reason. ``hbd.checkout`` is the
...
is green on a test run that happens to import ``hbd.checkout`` first and red on one that does
...
#: What this module is allowed to see. Everything ``hbd.checkout``'s docstring names, and
...
_PERMITTED_FIRST_PARTY: Final[frozenset[str]] = frozenset(
    {"hbd.contracts", "hbd.entitlements", "hbd.logging"}
)

#: The two that must never appear, spelled out so a failure names the rule it broke.
_FORBIDDEN_PREFIXES: Final[tuple[str, ...]] = ("hbd.db", "hbd.payme")


def _imported_modules() -> set[str]:
    """Every module ``hbd/checkout.py`` imports, including under ``if TYPE_CHECKING``.
    ...
    """
    source = Path(hbd.checkout.__file__).read_text(encoding="utf-8")
    modules: set[str] = set()
...
def test_the_buying_seam_never_imports_the_persistence_package() -> None:
    # Arrange / Act
    offenders = _matching(_imported_modules(), "hbd.db")

    # Assert — persistence implements this module's protocols from the other direction, the
    # way ``hbd.db.credits.SqlCreditLedger`` implements ``EntitlementStore``. An import here
    # is a cycle, and a cycle in a composition root fails at boot, not in a test.
    assert offenders == [], f"hbd.checkout must never import hbd.db; found {offenders}"


def test_the_buying_seam_never_imports_a_rail_package() -> None:
    # Arrange / Act
    offenders = _matching(_imported_modules(), "hbd.payme")
    ...
    assert offenders == [], f"hbd.checkout must never import a rail package; found {offenders}"


def test_the_buying_seam_imports_only_the_leaves_its_docstring_names() -> None:
    # Arrange / Act
    first_party = {name for name in _imported_modules() if name.split(".")[0] == "hbd"}
    ...
    assert first_party <= _PERMITTED_FIRST_PARTY, (
        "hbd.checkout grew a first-party dependency its docstring does not name: "
...
def test_the_forbidden_packages_still_exist_under_the_names_this_file_checks() -> None:
    ...
    # dies without going red. ``hbd.db`` is on disk today; ``hbd.payme`` is not yet, so it is
    ...
    packages = Path(hbd.checkout.__file__).parent

    # Assert
    assert (packages / "db").is_dir(), "hbd.db has moved; the prefix above now matches nothing"
    for prefix in _FORBIDDEN_PREFIXES:
        assert prefix.startswith("hbd."), prefix
