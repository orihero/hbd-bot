...
export const ru: TranslationSchema = {
  ...
  auth: {
    ...
    loginSubtitle: "Учетные данные оператора консоли HBD Bot.",
    loginSubtitleIssued:
    ...
    consoleTitle: "Операционная консоль HBD Bot",
    consoleDescription:
      "Обслуживание клиентов, конвейер генерации песен, журнал аудита и конфигурация системы Happy Birthday Bot.",
    ...
    login: {
      ...
      subtitle: "Учетные данные оператора консоли HBD Bot.",
    ...
    console: {
      heroTitle: "Операторская консоль бота hbd для песен-поздравлений",
      heroDescription:
  ...
  admins: {
    ...
    noWritesNote:
      "В этой сборке изменение учетных записей администраторов отключено. CLI (hbd.admin.bootstrap) только создает первого Владельца или восстанавливает права, когда активных Владельцев не осталось, а свой пароль вы меняете в этой консоли. Добавление операторов, смена ролей, сброс чужих паролей и деактивация пока не поддерживаются ни здесь, ни через CLI.",
  ...
  errors: {
    ...
    detail: {
      ...
      misconfiguredMessage:
        "API отклонил источник запроса. Это ошибка развёртывания, а не ваше действие, и повтор её не исправит — HBD_ADMIN_PUBLIC_ORIGIN не совпадает с адресом, откуда отдаётся эта панель.",
    ...
    query: {
      ...
      originMessage:
        "{message} Это настройка развёртывания (HBD_ADMIN_PUBLIC_ORIGIN), а не ваше действие — повтор ничего не изменит.",
